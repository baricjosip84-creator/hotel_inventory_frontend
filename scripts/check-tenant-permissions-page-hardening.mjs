...
includes(editorCss, 'grid-template-columns: repeat(2, minmax(0, 1fr));', 'two-column collapsed permission group layout');
includes(editorCss, '.role-permission-editor__group-card.is-expanded', 'full-width expanded permission group layout');
includes(editorCss, '.role-permission-editor__empty-search', 'full-width permission search result state');
...
includes(tenantCss, '.tenant-permissions-role-card__manage', 'tenant custom role selection affordance CSS');
...
console.log('Tenant permissions page hardening check passed (shared operational workspace UI, two-column collapsed permission groups, full-width expansion, explicit custom-role management affordance, dependency-safe editing, draft discard, and lifecycle guards verified).');
