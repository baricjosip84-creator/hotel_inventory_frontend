...
const checks = [
  ...
  ['mandatory multilingual closure remains first and v3.49.185 runs immediately after it', packageJson.scripts?.['check:ci']?.startsWith('npm run check:tenant-multilingual-closure-audit && npm run check:command-pages-final-business-presentation-closure-v349186 && npm run check:adaptive-policy-source-workflow-presentation-closure-v349185 && npm run check:learning-feedback-business-presentation-closure-v349184 && npm run check:adaptive-policy-business-presentation-closure-v349183 && ')]
];
