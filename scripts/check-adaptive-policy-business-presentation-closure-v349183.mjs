...
const checks = [
  ...
  ['v3.49.183 guard is wired into frontend CI after the mandatory multilingual closure audit', packageJson.scripts?.['check:adaptive-policy-business-presentation-closure-v349183'] === 'node scripts/check-adaptive-policy-business-presentation-closure-v349183.mjs' && packageJson.scripts?.['check:ci']?.startsWith('npm run check:tenant-multilingual-closure-audit && npm run check:learning-feedback-business-presentation-closure-v349184 && npm run check:adaptive-policy-business-presentation-closure-v349183 && ')]
];
