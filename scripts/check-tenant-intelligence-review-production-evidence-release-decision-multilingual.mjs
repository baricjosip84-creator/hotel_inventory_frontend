...
const systemPresentation=['localizedReadinessSystemText(gap.feature_label, ui)','localizedReadinessSystemText(blocker.feature_label, ui)','localizedReadinessSystemText(blocker.required_resolution, ui)','releaseFinalEvidence.map((item)','localizedReadinessSystemText(item, ui)'];
for(const value of systemPresentation) if(!slice.includes(value)) fail(`System-owned evidence/release localization missing: ${value}`);
const preservedData=['gap.table_name','blocker.detail','evidenceMatrixQuery.error.message','releaseDecisionBoardQuery.error.message'];
for(const value of preservedData) if(!slice.includes(value)) fail(`Evidence/release technical or error boundary missing: ${value}`);
for(const value of ['ui(gap.table_name)','ui(evidenceMatrixQuery.error.message)','ui(releaseDecisionBoardQuery.error.message)','localizedReadinessSystemText(gap.table_name']) if(slice.includes(value)) fail(`Technical evidence/API errors must remain untranslated: ${value}`);
if(!process.exitCode) pass('System-owned feature/resolution/final-test guidance is localized while table names, technical blocker evidence, and API errors remain data.');
