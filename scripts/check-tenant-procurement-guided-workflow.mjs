...
for (const text of ['How Purchase Orders work','How Shipments work','Shipment reference','No eligible Purchase Orders for this supplier','Purchase price per unit','Line total',"This is the supplier's purchase price for this order, not the product standard cost."]) expect(catalog.includes(`[${JSON.stringify(text)}`) || catalog.includes(`['${text}'`), `translation catalog includes ${text}`);
console.log(`Tenant procurement guided workflow v3.49.115: PASS (${checks.length}/${checks.length} checks).`);
