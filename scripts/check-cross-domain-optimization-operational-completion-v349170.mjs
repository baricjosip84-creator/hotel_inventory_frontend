...
check(page.includes('Advanced governance settings') && page.includes("method: 'PUT'") && page.includes('Save governance settings'), 'Tenant governance settings editor is wired through the current collapsed Advanced governance settings presentation.');
check(page.includes('Ownership and next action') && page.includes("action: 'update_ownership'") && page.includes('projectedSourceWorkflowPath') && page.includes("ui('Open Replenishment Planning')") && page.includes("ui('Open Execution Tasks')"), 'Owner, due date, next action, and authoritative source-workflow handoffs are wired.');
