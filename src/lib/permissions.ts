import { getAccessToken } from './auth';

export type UserRole =
  | 'admin'
  | 'manager'
  | 'staff'
  | 'support_read_only'
  | 'support_inventory'
  | 'support_procurement'
  | 'unknown';

export const TENANT_PERMISSIONS = Object.freeze({
  DASHBOARD_READ: 'dashboard.read',
  PRODUCTS_READ: 'products.read',
  PRODUCTS_WRITE: 'products.write',
  PRODUCT_PACKAGES_READ: 'product_packages.read',
  PRODUCT_PACKAGES_WRITE: 'product_packages.write',
  SUPPLIERS_READ: 'suppliers.read',
  SUPPLIERS_WRITE: 'suppliers.write',
  STORAGE_LOCATIONS_READ: 'storage_locations.read',
  STORAGE_LOCATIONS_WRITE: 'storage_locations.write',
  STOCK_READ: 'stock.read',
  STOCK_CONSUME: 'stock.consume',
  STOCK_COUNT: 'stock.count',
  STOCK_ADJUST: 'stock.adjust',
  STOCK_MOVEMENTS_READ: 'stock_movements.read',
  STOCK_TRANSFERS_READ: 'stock_transfers.read',
  STOCK_TRANSFERS_CREATE: 'stock_transfers.create',
  STOCK_TRANSFERS_UPDATE: 'stock_transfers.update',
  STOCK_TRANSFERS_EXECUTE: 'stock_transfers.execute',
  STOCK_TRANSFERS_CANCEL: 'stock_transfers.cancel',
  INVENTORY_USAGE_READ: 'inventory_usage.read',
  INVENTORY_USAGE_RECORD: 'inventory_usage.record',
  INVENTORY_USAGE_BULK_RECORD: 'inventory_usage.bulk_record',
  INVENTORY_USAGE_REVERSE: 'inventory_usage.reverse',
  INVENTORY_USAGE_REVIEW: 'inventory_usage.review',
  INVENTORY_USAGE_CLOSE_PERIOD: 'inventory_usage.close_period',
  INVENTORY_USAGE_MANAGE_TEMPLATES: 'inventory_usage.manage_templates',
  INVENTORY_USAGE_RUN_SCHEDULED: 'inventory_usage.run_scheduled',
  INVENTORY_REQUISITIONS_READ: 'inventory_requisitions.read',
  INVENTORY_REQUISITIONS_CREATE: 'inventory_requisitions.create',
  INVENTORY_REQUISITIONS_SUBMIT: 'inventory_requisitions.submit',
  INVENTORY_REQUISITIONS_APPROVE: 'inventory_requisitions.approve',
  INVENTORY_REQUISITIONS_FULFILL: 'inventory_requisitions.fulfill',
  INVENTORY_REQUISITIONS_CANCEL_OWN: 'inventory_requisitions.cancel_own',
  INVENTORY_REQUISITIONS_CANCEL_ANY: 'inventory_requisitions.cancel_any',
  INVENTORY_RESERVATIONS_READ: 'inventory_reservations.read',
  INVENTORY_RESERVATIONS_CREATE: 'inventory_reservations.create',
  INVENTORY_RESERVATIONS_ALLOCATE: 'inventory_reservations.allocate',
  INVENTORY_RESERVATIONS_FULFILL: 'inventory_reservations.fulfill',
  INVENTORY_RESERVATIONS_RELEASE: 'inventory_reservations.release',
  INVENTORY_RESERVATIONS_CANCEL_OWN: 'inventory_reservations.cancel_own',
  INVENTORY_RESERVATIONS_CANCEL_ANY: 'inventory_reservations.cancel_any',
  INVENTORY_RESERVATIONS_EXPIRE: 'inventory_reservations.expire',
  PURCHASE_ORDERS_READ: 'purchase_orders.read',
  PURCHASE_ORDERS_CREATE: 'purchase_orders.create',
  PURCHASE_ORDERS_UPDATE: 'purchase_orders.update',
  PURCHASE_ORDERS_SUBMIT: 'purchase_orders.submit',
  PURCHASE_ORDERS_APPROVE: 'purchase_orders.approve',
  PURCHASE_ORDERS_CANCEL: 'purchase_orders.cancel',
  SHIPMENTS_READ: 'shipments.read',
  SHIPMENTS_WRITE: 'shipments.write',
  SHIPMENTS_SEND: 'shipments.send',
  SHIPMENTS_RECEIVE: 'shipments.receive',
  SHIPMENTS_FINALIZE: 'shipments.finalize',
  SHIPMENTS_AUTO_REORDER: 'shipments.auto_reorder',
  SHIPMENT_ITEMS_READ: 'shipment_items.read',
  SHIPMENT_ITEMS_WRITE: 'shipment_items.write',
  ALERTS_READ: 'alerts.read',
  ALERTS_WRITE: 'alerts.write',
  ALERTS_OVERRIDE: 'alerts.override',
  REPORTS_READ: 'reports.read',
  INSIGHTS_READ: 'insights.read',
  TENANT_READ: 'tenant.read',
  TENANT_UPDATE: 'tenant.update',
  TENANT_DELETE: 'tenant.delete',
  USERS_READ: 'users.read',
  USERS_WRITE: 'users.write',
  SYSTEM_STATUS_READ: 'system_status.read',
  SYSTEM_CONTEXT_READ: 'system_context.read',
  EXECUTION_REQUESTS_VIEW: 'execution_requests.view',
  EXECUTION_REQUESTS_CREATE: 'execution_requests.create',
  EXECUTION_REQUESTS_SUBMIT: 'execution_requests.submit',
  EXECUTION_REQUESTS_CANCEL: 'execution_requests.cancel',
  EXECUTION_REQUESTS_REVIEW: 'execution_requests.review',
  EXECUTION_REQUESTS_EXECUTE: 'execution_requests.execute',
  EXECUTION_TASKS_READ: 'execution_tasks.read',
  EXECUTION_TASKS_CREATE: 'execution_tasks.create',
  EXECUTION_TASKS_ASSIGN: 'execution_tasks.assign',
  EXECUTION_TASKS_UPDATE: 'execution_tasks.update',
  EXECUTION_TASKS_CANCEL: 'execution_tasks.cancel',
  EXECUTION_TASKS_COMPLETE: 'execution_tasks.complete',
  INVENTORY_OPTIMIZATION_READ: 'inventory_optimization.read',
  INVENTORY_OPTIMIZATION_CREATE: 'inventory_optimization.create',
  INVENTORY_OPTIMIZATION_GOVERN: 'inventory_optimization.govern',
  CONTROL_TOWER_READ: 'control_tower.read',
  CONTROL_TOWER_CREATE: 'control_tower.create',
  CONTROL_TOWER_GOVERN: 'control_tower.govern',
  FINANCIAL_INTELLIGENCE_READ: 'financial_intelligence.read',
  FINANCIAL_INTELLIGENCE_GOVERN: 'financial_intelligence.govern',
  ENTERPRISE_INTEGRATIONS_READ: 'enterprise_integrations.read',
  ENTERPRISE_INTEGRATIONS_GOVERN: 'enterprise_integrations.govern',
  DECISION_INTELLIGENCE_READ: 'decision_intelligence.read',
  DECISION_INTELLIGENCE_GOVERN: 'decision_intelligence.govern',
  DECISION_INTELLIGENCE_REVIEW_OVERRIDE: 'decision_intelligence.review_override',
  OPERATIONAL_ACTION_CENTER_READ: 'operational_action_center.read',
  PLATFORM_RELIABILITY_READ: 'platform_reliability.read',
  AUTOMATION_SCHEDULES_VIEW: 'automation_schedules.view',
  AUTOMATION_SCHEDULES_CREATE: 'automation_schedules.create',
  AUTOMATION_SCHEDULES_UPDATE: 'automation_schedules.update',
  AUTOMATION_SCHEDULES_PAUSE: 'automation_schedules.pause',
  AUTOMATION_SCHEDULES_RESUME: 'automation_schedules.resume',
  AUTOMATION_SCHEDULES_DISABLE: 'automation_schedules.disable',
  TENANT_DIAGNOSTICS_READ: 'tenant_diagnostics.read',
  PAR_LEVELS_READ: 'par_levels.read',
  PAR_LEVELS_WRITE: 'par_levels.write',
  CYCLE_COUNTS_READ: 'cycle_counts.read',
  CYCLE_COUNTS_WRITE: 'cycle_counts.write',
  CYCLE_COUNTS_APPROVE: 'cycle_counts.approve',
  REQUISITIONS_READ: 'requisitions.read',
  REQUISITIONS_CREATE: 'requisitions.create',
  SUPPLIER_RETURNS_READ: 'supplier_returns.read',
  SUPPLIER_RETURNS_WRITE: 'supplier_returns.write',
  SUPPLIER_RETURNS_DISPATCH: 'supplier_returns.dispatch',
  APPROVAL_RULES_READ: 'approval_rules.read',
  APPROVAL_RULES_WRITE: 'approval_rules.write',
  APPROVALS_EXECUTE: 'approvals.execute',
  SUPPLIER_CATALOG_READ: 'supplier_catalog.read',
  SUPPLIER_CATALOG_WRITE: 'supplier_catalog.write',
  INVOICES_READ: 'invoices.read',
  INVOICES_WRITE: 'invoices.write',
  INVOICES_MATCH: 'invoices.match',
  INVOICES_MARK_PAID: 'invoices.mark_paid',
  ATTACHMENTS_READ: 'attachments.read',
  ATTACHMENTS_WRITE: 'attachments.write',
  NOTIFICATIONS_READ: 'notifications.read',
  NOTIFICATIONS_WRITE: 'notifications.write',
  BARCODE_LABELS_READ: 'barcode_labels.read',
  BARCODE_LABELS_WRITE: 'barcode_labels.write',
  AUDIT_READ: 'audit.read',
  ROLE_PERMISSIONS_READ: 'role_permissions.read',
  ROLE_PERMISSIONS_WRITE: 'role_permissions.write',
  CUSTOMERS_READ: 'customers.read',
  CUSTOMERS_WRITE: 'customers.write',
  OUTBOUND_ORDERS_READ: 'outbound_orders.read',
  OUTBOUND_ORDERS_CREATE: 'outbound_orders.create',
  OUTBOUND_ORDERS_UPDATE: 'outbound_orders.update',
  OUTBOUND_ORDERS_DISPATCH: 'outbound_orders.dispatch',
  OUTBOUND_ORDERS_CANCEL: 'outbound_orders.cancel',
  CUSTOMER_RETURNS_READ: 'customer_returns.read',
  CUSTOMER_RETURNS_CREATE: 'customer_returns.create',
  CUSTOMER_RETURNS_RECEIVE: 'customer_returns.receive',
  CUSTOMER_RETURNS_CANCEL: 'customer_returns.cancel',
} as const);

export type TenantPermission = (typeof TENANT_PERMISSIONS)[keyof typeof TENANT_PERMISSIONS];

type JwtPayload = {
  role?: string;
  tenant_id?: string;
  id?: string;
  custom_role_id?: string;
  custom_role_name?: string;
  exp?: number;
};

const KNOWN_USER_ROLES: ReadonlySet<Exclude<UserRole, 'unknown'>> = new Set([
  'admin',
  'manager',
  'staff',
  'support_read_only',
  'support_inventory',
  'support_procurement'
]);

function isKnownUserRole(value: unknown): value is Exclude<UserRole, 'unknown'> {
  return typeof value === 'string' && KNOWN_USER_ROLES.has(value as Exclude<UserRole, 'unknown'>);
}

function decodeJwtPayload(token: string | null): JwtPayload | null {
  if (!token) {
    return null;
  }

  try {
    const parts = token.split('.');

    if (parts.length !== 3) {
      return null;
    }

    const base64 = parts[1].replace(/-/g, '+').replace(/_/g, '/');
    const padded = base64.padEnd(Math.ceil(base64.length / 4) * 4, '=');
    const decoded = atob(padded);

    return JSON.parse(decoded) as JwtPayload;
  } catch {
    return null;
  }
}

export const ROLE_PERMISSIONS: Record<Exclude<UserRole, 'unknown'>, readonly TenantPermission[]> = Object.freeze({
  admin: Object.freeze([
    TENANT_PERMISSIONS.DASHBOARD_READ,
    TENANT_PERMISSIONS.PRODUCTS_READ,
    TENANT_PERMISSIONS.PRODUCTS_WRITE,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_READ,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_WRITE,
    TENANT_PERMISSIONS.SUPPLIERS_READ,
    TENANT_PERMISSIONS.SUPPLIERS_WRITE,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_READ,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_WRITE,
    TENANT_PERMISSIONS.STOCK_READ,
    TENANT_PERMISSIONS.STOCK_COUNT,
    TENANT_PERMISSIONS.STOCK_ADJUST,
    TENANT_PERMISSIONS.STOCK_MOVEMENTS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_CREATE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_UPDATE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_EXECUTE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_CANCEL,
    TENANT_PERMISSIONS.INVENTORY_USAGE_READ,
    TENANT_PERMISSIONS.INVENTORY_USAGE_REVERSE,
    TENANT_PERMISSIONS.INVENTORY_USAGE_REVIEW,
    TENANT_PERMISSIONS.INVENTORY_USAGE_CLOSE_PERIOD,
    TENANT_PERMISSIONS.INVENTORY_USAGE_MANAGE_TEMPLATES,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CREATE,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_SUBMIT,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_APPROVE,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_FULFILL,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_OWN,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_ANY,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CREATE,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_ALLOCATE,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_FULFILL,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_RELEASE,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_OWN,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_ANY,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_EXPIRE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_READ,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_CREATE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_UPDATE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_SUBMIT,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_APPROVE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_CANCEL,
    TENANT_PERMISSIONS.SHIPMENTS_READ,
    TENANT_PERMISSIONS.SHIPMENTS_WRITE,
    TENANT_PERMISSIONS.SHIPMENTS_SEND,
    TENANT_PERMISSIONS.SHIPMENTS_RECEIVE,
    TENANT_PERMISSIONS.SHIPMENTS_FINALIZE,
    TENANT_PERMISSIONS.SHIPMENTS_AUTO_REORDER,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_READ,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_WRITE,
    TENANT_PERMISSIONS.ALERTS_READ,
    TENANT_PERMISSIONS.ALERTS_WRITE,
    TENANT_PERMISSIONS.ALERTS_OVERRIDE,
    TENANT_PERMISSIONS.REPORTS_READ,
    TENANT_PERMISSIONS.INSIGHTS_READ,
    TENANT_PERMISSIONS.PAR_LEVELS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_WRITE,
    TENANT_PERMISSIONS.REQUISITIONS_READ,
    TENANT_PERMISSIONS.REQUISITIONS_CREATE,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_READ,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_WRITE,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_DISPATCH,
    TENANT_PERMISSIONS.SUPPLIER_CATALOG_READ,
    TENANT_PERMISSIONS.ATTACHMENTS_READ,
    TENANT_PERMISSIONS.ATTACHMENTS_WRITE,
    TENANT_PERMISSIONS.NOTIFICATIONS_READ,
    TENANT_PERMISSIONS.BARCODE_LABELS_READ,
    TENANT_PERMISSIONS.CUSTOMERS_READ,
    TENANT_PERMISSIONS.CUSTOMERS_WRITE,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_READ,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_CREATE,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_UPDATE,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_DISPATCH,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_CANCEL,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_READ,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_CREATE,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_RECEIVE,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_CANCEL,
    TENANT_PERMISSIONS.TENANT_READ,
    TENANT_PERMISSIONS.TENANT_UPDATE,
    TENANT_PERMISSIONS.TENANT_DELETE,
    TENANT_PERMISSIONS.USERS_READ,
    TENANT_PERMISSIONS.USERS_WRITE,
    TENANT_PERMISSIONS.SYSTEM_STATUS_READ,
    TENANT_PERMISSIONS.SYSTEM_CONTEXT_READ,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_VIEW,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_CREATE,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_SUBMIT,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_CANCEL,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_REVIEW,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_EXECUTE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_READ,
    TENANT_PERMISSIONS.EXECUTION_TASKS_CREATE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_ASSIGN,
    TENANT_PERMISSIONS.EXECUTION_TASKS_UPDATE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_CANCEL,
    TENANT_PERMISSIONS.EXECUTION_TASKS_COMPLETE,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_READ,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_CREATE,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_GOVERN,
    TENANT_PERMISSIONS.CONTROL_TOWER_READ,
    TENANT_PERMISSIONS.CONTROL_TOWER_CREATE,
    TENANT_PERMISSIONS.CONTROL_TOWER_GOVERN,
    TENANT_PERMISSIONS.FINANCIAL_INTELLIGENCE_READ,
    TENANT_PERMISSIONS.FINANCIAL_INTELLIGENCE_GOVERN,
    TENANT_PERMISSIONS.ENTERPRISE_INTEGRATIONS_READ,
    TENANT_PERMISSIONS.ENTERPRISE_INTEGRATIONS_GOVERN,
    TENANT_PERMISSIONS.DECISION_INTELLIGENCE_READ,
    TENANT_PERMISSIONS.DECISION_INTELLIGENCE_GOVERN,
    TENANT_PERMISSIONS.DECISION_INTELLIGENCE_REVIEW_OVERRIDE,
    TENANT_PERMISSIONS.OPERATIONAL_ACTION_CENTER_READ,
    TENANT_PERMISSIONS.PLATFORM_RELIABILITY_READ,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_VIEW,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_CREATE,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_UPDATE,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_PAUSE,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_RESUME,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_DISABLE,
    TENANT_PERMISSIONS.TENANT_DIAGNOSTICS_READ,
    TENANT_PERMISSIONS.PAR_LEVELS_WRITE,
    TENANT_PERMISSIONS.CYCLE_COUNTS_APPROVE,
    TENANT_PERMISSIONS.APPROVAL_RULES_READ,
    TENANT_PERMISSIONS.APPROVAL_RULES_WRITE,
    TENANT_PERMISSIONS.APPROVALS_EXECUTE,
    TENANT_PERMISSIONS.SUPPLIER_CATALOG_WRITE,
    TENANT_PERMISSIONS.INVOICES_READ,
    TENANT_PERMISSIONS.INVOICES_WRITE,
    TENANT_PERMISSIONS.INVOICES_MATCH,
    TENANT_PERMISSIONS.INVOICES_MARK_PAID,
    TENANT_PERMISSIONS.NOTIFICATIONS_WRITE,
    TENANT_PERMISSIONS.BARCODE_LABELS_WRITE,
    TENANT_PERMISSIONS.AUDIT_READ,
    TENANT_PERMISSIONS.ROLE_PERMISSIONS_READ,
    TENANT_PERMISSIONS.ROLE_PERMISSIONS_WRITE,
  ]),
  manager: Object.freeze([
    TENANT_PERMISSIONS.DASHBOARD_READ,
    TENANT_PERMISSIONS.PRODUCTS_READ,
    TENANT_PERMISSIONS.PRODUCTS_WRITE,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_READ,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_WRITE,
    TENANT_PERMISSIONS.SUPPLIERS_READ,
    TENANT_PERMISSIONS.SUPPLIERS_WRITE,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_READ,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_WRITE,
    TENANT_PERMISSIONS.STOCK_READ,
    TENANT_PERMISSIONS.STOCK_CONSUME,
    TENANT_PERMISSIONS.STOCK_COUNT,
    TENANT_PERMISSIONS.STOCK_ADJUST,
    TENANT_PERMISSIONS.STOCK_MOVEMENTS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_CREATE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_UPDATE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_EXECUTE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_CANCEL,
    TENANT_PERMISSIONS.CUSTOMERS_READ,
    TENANT_PERMISSIONS.CUSTOMERS_WRITE,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_READ,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_CREATE,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_UPDATE,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_DISPATCH,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_CANCEL,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_READ,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_CREATE,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_RECEIVE,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_CANCEL,
    TENANT_PERMISSIONS.INVENTORY_USAGE_READ,
    TENANT_PERMISSIONS.INVENTORY_USAGE_RECORD,
    TENANT_PERMISSIONS.INVENTORY_USAGE_BULK_RECORD,
    TENANT_PERMISSIONS.INVENTORY_USAGE_REVERSE,
    TENANT_PERMISSIONS.INVENTORY_USAGE_REVIEW,
    TENANT_PERMISSIONS.INVENTORY_USAGE_CLOSE_PERIOD,
    TENANT_PERMISSIONS.INVENTORY_USAGE_MANAGE_TEMPLATES,
    TENANT_PERMISSIONS.INVENTORY_USAGE_RUN_SCHEDULED,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CREATE,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_SUBMIT,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_APPROVE,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_FULFILL,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_OWN,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_ANY,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CREATE,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_ALLOCATE,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_FULFILL,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_RELEASE,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_OWN,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_ANY,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_EXPIRE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_READ,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_CREATE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_UPDATE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_SUBMIT,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_APPROVE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_CANCEL,
    TENANT_PERMISSIONS.SHIPMENTS_READ,
    TENANT_PERMISSIONS.SHIPMENTS_WRITE,
    TENANT_PERMISSIONS.SHIPMENTS_SEND,
    TENANT_PERMISSIONS.SHIPMENTS_RECEIVE,
    TENANT_PERMISSIONS.SHIPMENTS_FINALIZE,
    TENANT_PERMISSIONS.SHIPMENTS_AUTO_REORDER,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_READ,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_WRITE,
    TENANT_PERMISSIONS.ALERTS_READ,
    TENANT_PERMISSIONS.ALERTS_WRITE,
    TENANT_PERMISSIONS.REPORTS_READ,
    TENANT_PERMISSIONS.INSIGHTS_READ,
    TENANT_PERMISSIONS.TENANT_READ,
    TENANT_PERMISSIONS.USERS_READ,
    TENANT_PERMISSIONS.SYSTEM_STATUS_READ,
    TENANT_PERMISSIONS.SYSTEM_CONTEXT_READ,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_VIEW,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_CREATE,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_SUBMIT,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_CANCEL,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_REVIEW,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_EXECUTE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_READ,
    TENANT_PERMISSIONS.EXECUTION_TASKS_CREATE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_ASSIGN,
    TENANT_PERMISSIONS.EXECUTION_TASKS_UPDATE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_CANCEL,
    TENANT_PERMISSIONS.EXECUTION_TASKS_COMPLETE,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_READ,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_CREATE,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_GOVERN,
    TENANT_PERMISSIONS.CONTROL_TOWER_READ,
    TENANT_PERMISSIONS.CONTROL_TOWER_CREATE,
    TENANT_PERMISSIONS.CONTROL_TOWER_GOVERN,
    TENANT_PERMISSIONS.FINANCIAL_INTELLIGENCE_READ,
    TENANT_PERMISSIONS.FINANCIAL_INTELLIGENCE_GOVERN,
    TENANT_PERMISSIONS.ENTERPRISE_INTEGRATIONS_READ,
    TENANT_PERMISSIONS.ENTERPRISE_INTEGRATIONS_GOVERN,
    TENANT_PERMISSIONS.DECISION_INTELLIGENCE_READ,
    TENANT_PERMISSIONS.DECISION_INTELLIGENCE_GOVERN,
    TENANT_PERMISSIONS.OPERATIONAL_ACTION_CENTER_READ,
    TENANT_PERMISSIONS.PLATFORM_RELIABILITY_READ,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_VIEW,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_CREATE,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_UPDATE,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_PAUSE,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_RESUME,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_DISABLE,
    TENANT_PERMISSIONS.BARCODE_LABELS_READ,
    TENANT_PERMISSIONS.BARCODE_LABELS_WRITE,
    TENANT_PERMISSIONS.AUDIT_READ,
    TENANT_PERMISSIONS.PAR_LEVELS_READ,
    TENANT_PERMISSIONS.PAR_LEVELS_WRITE,
    TENANT_PERMISSIONS.CYCLE_COUNTS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_WRITE,
    TENANT_PERMISSIONS.CYCLE_COUNTS_APPROVE,
    TENANT_PERMISSIONS.REQUISITIONS_READ,
    TENANT_PERMISSIONS.REQUISITIONS_CREATE,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_READ,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_WRITE,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_DISPATCH,
    TENANT_PERMISSIONS.APPROVAL_RULES_READ,
    TENANT_PERMISSIONS.APPROVALS_EXECUTE,
    TENANT_PERMISSIONS.SUPPLIER_CATALOG_READ,
    TENANT_PERMISSIONS.SUPPLIER_CATALOG_WRITE,
    TENANT_PERMISSIONS.INVOICES_READ,
    TENANT_PERMISSIONS.INVOICES_WRITE,
    TENANT_PERMISSIONS.INVOICES_MATCH,
    TENANT_PERMISSIONS.ATTACHMENTS_READ,
    TENANT_PERMISSIONS.ATTACHMENTS_WRITE,
    TENANT_PERMISSIONS.NOTIFICATIONS_READ,
    TENANT_PERMISSIONS.NOTIFICATIONS_WRITE,
  ]),
  support_read_only: Object.freeze([
    TENANT_PERMISSIONS.DASHBOARD_READ,
    TENANT_PERMISSIONS.PRODUCTS_READ,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_READ,
    TENANT_PERMISSIONS.SUPPLIERS_READ,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_READ,
    TENANT_PERMISSIONS.STOCK_READ,
    TENANT_PERMISSIONS.STOCK_MOVEMENTS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_READ,
    TENANT_PERMISSIONS.INVENTORY_USAGE_READ,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_READ,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_READ,
    TENANT_PERMISSIONS.SHIPMENTS_READ,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_READ,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_READ,
    TENANT_PERMISSIONS.ALERTS_READ,
    TENANT_PERMISSIONS.REPORTS_READ,
    TENANT_PERMISSIONS.INSIGHTS_READ,
    TENANT_PERMISSIONS.TENANT_READ,
    TENANT_PERMISSIONS.SYSTEM_STATUS_READ,
    TENANT_PERMISSIONS.SYSTEM_CONTEXT_READ,
    TENANT_PERMISSIONS.PAR_LEVELS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_READ,
    TENANT_PERMISSIONS.REQUISITIONS_READ,
    TENANT_PERMISSIONS.SUPPLIER_CATALOG_READ,
    TENANT_PERMISSIONS.INVOICES_READ,
    TENANT_PERMISSIONS.ATTACHMENTS_READ,
    TENANT_PERMISSIONS.NOTIFICATIONS_READ,
    TENANT_PERMISSIONS.BARCODE_LABELS_READ,
    TENANT_PERMISSIONS.AUDIT_READ,
  ]),
  support_inventory: Object.freeze([
    TENANT_PERMISSIONS.DASHBOARD_READ,
    TENANT_PERMISSIONS.PRODUCTS_READ,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_READ,
    TENANT_PERMISSIONS.SUPPLIERS_READ,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_READ,
    TENANT_PERMISSIONS.STOCK_READ,
    TENANT_PERMISSIONS.STOCK_COUNT,
    TENANT_PERMISSIONS.STOCK_ADJUST,
    TENANT_PERMISSIONS.STOCK_MOVEMENTS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_CREATE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_UPDATE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_EXECUTE,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_CANCEL,
    TENANT_PERMISSIONS.INVENTORY_USAGE_READ,
    TENANT_PERMISSIONS.INVENTORY_USAGE_RECORD,
    TENANT_PERMISSIONS.INVENTORY_USAGE_BULK_RECORD,
    TENANT_PERMISSIONS.INVENTORY_USAGE_REVERSE,
    TENANT_PERMISSIONS.INVENTORY_USAGE_REVIEW,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_RELEASE,
    TENANT_PERMISSIONS.SHIPMENTS_READ,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_READ,
    TENANT_PERMISSIONS.SHIPMENTS_RECEIVE,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_READ,
    TENANT_PERMISSIONS.ALERTS_READ,
    TENANT_PERMISSIONS.REPORTS_READ,
    TENANT_PERMISSIONS.INSIGHTS_READ,
    TENANT_PERMISSIONS.TENANT_READ,
    TENANT_PERMISSIONS.SYSTEM_STATUS_READ,
    TENANT_PERMISSIONS.SYSTEM_CONTEXT_READ,
    TENANT_PERMISSIONS.PAR_LEVELS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_WRITE,
    TENANT_PERMISSIONS.REQUISITIONS_READ,
    TENANT_PERMISSIONS.SUPPLIER_CATALOG_READ,
    TENANT_PERMISSIONS.INVOICES_READ,
    TENANT_PERMISSIONS.ATTACHMENTS_READ,
    TENANT_PERMISSIONS.NOTIFICATIONS_READ,
    TENANT_PERMISSIONS.BARCODE_LABELS_READ,
    TENANT_PERMISSIONS.AUDIT_READ,
  ]),
  support_procurement: Object.freeze([
    TENANT_PERMISSIONS.DASHBOARD_READ,
    TENANT_PERMISSIONS.PRODUCTS_READ,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_READ,
    TENANT_PERMISSIONS.SUPPLIERS_READ,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_READ,
    TENANT_PERMISSIONS.STOCK_READ,
    TENANT_PERMISSIONS.STOCK_MOVEMENTS_READ,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_READ,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_CREATE,
    TENANT_PERMISSIONS.PURCHASE_ORDERS_UPDATE,
    TENANT_PERMISSIONS.SHIPMENTS_READ,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_READ,
    TENANT_PERMISSIONS.SHIPMENTS_WRITE,
    TENANT_PERMISSIONS.SHIPMENTS_SEND,
    TENANT_PERMISSIONS.SHIPMENTS_RECEIVE,
    TENANT_PERMISSIONS.SHIPMENTS_FINALIZE,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_READ,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_WRITE,
    TENANT_PERMISSIONS.ALERTS_READ,
    TENANT_PERMISSIONS.REPORTS_READ,
    TENANT_PERMISSIONS.INSIGHTS_READ,
    TENANT_PERMISSIONS.TENANT_READ,
    TENANT_PERMISSIONS.SYSTEM_STATUS_READ,
    TENANT_PERMISSIONS.SYSTEM_CONTEXT_READ,
    TENANT_PERMISSIONS.SUPPLIER_CATALOG_READ,
    TENANT_PERMISSIONS.INVOICES_READ,
    TENANT_PERMISSIONS.INVOICES_WRITE,
    TENANT_PERMISSIONS.ATTACHMENTS_READ,
    TENANT_PERMISSIONS.NOTIFICATIONS_READ,
    TENANT_PERMISSIONS.BARCODE_LABELS_READ,
    TENANT_PERMISSIONS.AUDIT_READ,
  ]),
  staff: Object.freeze([
    TENANT_PERMISSIONS.DASHBOARD_READ,
    TENANT_PERMISSIONS.PRODUCTS_READ,
    TENANT_PERMISSIONS.PRODUCT_PACKAGES_READ,
    TENANT_PERMISSIONS.SUPPLIERS_READ,
    TENANT_PERMISSIONS.STORAGE_LOCATIONS_READ,
    TENANT_PERMISSIONS.STOCK_READ,
    TENANT_PERMISSIONS.STOCK_CONSUME,
    TENANT_PERMISSIONS.STOCK_COUNT,
    TENANT_PERMISSIONS.STOCK_MOVEMENTS_READ,
    TENANT_PERMISSIONS.CUSTOMERS_READ,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_READ,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_UPDATE,
    TENANT_PERMISSIONS.OUTBOUND_ORDERS_DISPATCH,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_READ,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_CREATE,
    TENANT_PERMISSIONS.CUSTOMER_RETURNS_RECEIVE,
    TENANT_PERMISSIONS.INVENTORY_USAGE_READ,
    TENANT_PERMISSIONS.INVENTORY_USAGE_RECORD,
    TENANT_PERMISSIONS.INVENTORY_USAGE_BULK_RECORD,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_READ,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CREATE,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_SUBMIT,
    TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_OWN,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_READ,
    TENANT_PERMISSIONS.STOCK_TRANSFERS_READ,
    TENANT_PERMISSIONS.SHIPMENTS_READ,
    TENANT_PERMISSIONS.SHIPMENTS_RECEIVE,
    TENANT_PERMISSIONS.SHIPMENT_ITEMS_READ,
    TENANT_PERMISSIONS.ALERTS_READ,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_VIEW,
    TENANT_PERMISSIONS.EXECUTION_REQUESTS_CREATE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_READ,
    TENANT_PERMISSIONS.EXECUTION_TASKS_UPDATE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_COMPLETE,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_READ,
    TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_CREATE,
    TENANT_PERMISSIONS.CONTROL_TOWER_READ,
    TENANT_PERMISSIONS.CONTROL_TOWER_CREATE,
    TENANT_PERMISSIONS.FINANCIAL_INTELLIGENCE_READ,
    TENANT_PERMISSIONS.ENTERPRISE_INTEGRATIONS_READ,
    TENANT_PERMISSIONS.DECISION_INTELLIGENCE_READ,
    TENANT_PERMISSIONS.OPERATIONAL_ACTION_CENTER_READ,
    TENANT_PERMISSIONS.PLATFORM_RELIABILITY_READ,
    TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_VIEW,
    TENANT_PERMISSIONS.TENANT_READ,
    TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CREATE,
    TENANT_PERMISSIONS.EXECUTION_TASKS_CREATE,
    TENANT_PERMISSIONS.PAR_LEVELS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_READ,
    TENANT_PERMISSIONS.CYCLE_COUNTS_WRITE,
    TENANT_PERMISSIONS.REQUISITIONS_READ,
    TENANT_PERMISSIONS.REQUISITIONS_CREATE,
    TENANT_PERMISSIONS.SUPPLIER_RETURNS_READ,
    TENANT_PERMISSIONS.NOTIFICATIONS_READ,
    TENANT_PERMISSIONS.BARCODE_LABELS_READ,
  ]),
});

const TENANT_PERMISSION_SNAPSHOT_KEY = 'inventory_tenant_effective_permissions';
export const TENANT_PERMISSION_SNAPSHOT_EVENT = 'inventory-tenant-permissions-changed';

export type TenantPermissionSnapshot = {
  tenant_id: string;
  user_id?: string | null;
  role: UserRole;
  custom_role_id?: string | null;
  custom_role_name?: string | null;
  access_role?: string | null;
  access_role_label?: string | null;
  permissions: TenantPermission[];
  loaded_at: string;
};

function currentTenantIdentity(): { tenantId: string | null; userId: string | null; role: UserRole; customRoleId: string | null } {
  const payload = decodeJwtPayload(getAccessToken());
  const role: UserRole = isKnownUserRole(payload?.role) ? payload.role : 'unknown';

  return {
    tenantId: typeof payload?.tenant_id === 'string' ? payload.tenant_id : null,
    userId: typeof payload?.id === 'string' ? payload.id : null,
    role,
    customRoleId: typeof payload?.custom_role_id === 'string' ? payload.custom_role_id : null
  };
}

export function getTenantPermissionSnapshot(): TenantPermissionSnapshot | null {
  try {
    const raw = localStorage.getItem(TENANT_PERMISSION_SNAPSHOT_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<TenantPermissionSnapshot>;
    const validPermissions = new Set<string>(Object.values(TENANT_PERMISSIONS));
    if (
      typeof parsed.tenant_id !== 'string' ||
      !isKnownUserRole(parsed.role) ||
      !Array.isArray(parsed.permissions)
    ) {
      return null;
    }

    return {
      tenant_id: parsed.tenant_id,
      user_id: typeof parsed.user_id === 'string' || parsed.user_id === null ? parsed.user_id : null,
      role: parsed.role,
      custom_role_id: typeof parsed.custom_role_id === 'string' || parsed.custom_role_id === null ? parsed.custom_role_id : null,
      custom_role_name: typeof parsed.custom_role_name === 'string' || parsed.custom_role_name === null ? parsed.custom_role_name : null,
      access_role: typeof parsed.access_role === 'string' || parsed.access_role === null ? parsed.access_role : null,
      access_role_label: typeof parsed.access_role_label === 'string' || parsed.access_role_label === null ? parsed.access_role_label : null,
      permissions: [...new Set(parsed.permissions.filter((permission): permission is TenantPermission => validPermissions.has(String(permission))))],
      loaded_at: typeof parsed.loaded_at === 'string' ? parsed.loaded_at : new Date(0).toISOString()
    };
  } catch {
    return null;
  }
}

export function setTenantPermissionSnapshot(snapshot: Omit<TenantPermissionSnapshot, 'loaded_at'> & { loaded_at?: string }): void {
  const normalized: TenantPermissionSnapshot = {
    ...snapshot,
    permissions: [...new Set(snapshot.permissions)],
    loaded_at: snapshot.loaded_at || new Date().toISOString()
  };
  localStorage.setItem(TENANT_PERMISSION_SNAPSHOT_KEY, JSON.stringify(normalized));
  window.dispatchEvent(new CustomEvent(TENANT_PERMISSION_SNAPSHOT_EVENT));
}

export function clearTenantPermissionSnapshot(): void {
  localStorage.removeItem(TENANT_PERMISSION_SNAPSHOT_KEY);
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent(TENANT_PERMISSION_SNAPSHOT_EVENT));
  }
}

function snapshotMatchesCurrentTenantIdentity(snapshot: TenantPermissionSnapshot | null): snapshot is TenantPermissionSnapshot {
  if (!snapshot) return false;
  const identity = currentTenantIdentity();
  if (!identity.tenantId || identity.tenantId !== snapshot.tenant_id) return false;
  if (identity.userId && snapshot.user_id && identity.userId !== snapshot.user_id) return false;
  return true;
}

function cachedPermissionsForCurrentRole(role: UserRole): readonly TenantPermission[] | null {
  const snapshot = getTenantPermissionSnapshot();
  if (!snapshotMatchesCurrentTenantIdentity(snapshot) || role === 'unknown') return null;

  /*
    The permission snapshot comes from /permissions/me after backend auth has
    reloaded the user's current database role/custom role. The JWT can therefore
    be older than the current role until the next access-token refresh. Do not
    reject the authoritative snapshot merely because those token claims are
    stale; tenant + user identity is the stable boundary that must match.
  */
  if (snapshot.role !== role) return null;
  return snapshot.permissions;
}

export function getCurrentUserRole(): UserRole {
  const snapshot = getTenantPermissionSnapshot();
  if (snapshotMatchesCurrentTenantIdentity(snapshot) && isKnownUserRole(snapshot.role)) {
    return snapshot.role;
  }

  const payload = decodeJwtPayload(getAccessToken());
  const role = payload?.role;

  return isKnownUserRole(role) ? role : 'unknown';
}

export function getCurrentAccessRoleLabel(): string {
  const snapshot = getTenantPermissionSnapshot();

  if (snapshotMatchesCurrentTenantIdentity(snapshot)) {
    if (snapshot.access_role_label?.trim()) {
      return snapshot.access_role_label.trim();
    }

    if (snapshot.custom_role_name?.trim()) {
      return snapshot.custom_role_name.trim();
    }
  }

  const payload = decodeJwtPayload(getAccessToken());

  if (payload?.custom_role_name?.trim()) {
    return payload.custom_role_name.trim();
  }

  return isKnownUserRole(payload?.role) ? payload.role : 'unknown';
}

export function permissionsForRole(role: UserRole = getCurrentUserRole()): readonly TenantPermission[] {
  if (isKnownUserRole(role)) {
    const cached = cachedPermissionsForCurrentRole(role);
    if (cached) return cached;

    /*
      Once an authenticated tenant identity exists, effective permissions must
      come from /permissions/me. Falling back to built-in role defaults after a
      transient snapshot failure can make the UI appear more permissive than
      the backend's current database policy. Fail closed until the authoritative
      snapshot is available again.
    */
    const identity = currentTenantIdentity();
    if (identity.tenantId || identity.userId) return [];

    return [...new Set(ROLE_PERMISSIONS[role])];
  }

  return [];
}

export function hasPermission(permission: TenantPermission, role: UserRole = getCurrentUserRole()): boolean {
  return permissionsForRole(role).includes(permission);
}

export function hasAllPermissions(permissions: TenantPermission[], role: UserRole = getCurrentUserRole()): boolean {
  return permissions.every((permission) => hasPermission(permission, role));
}

export function hasAnyPermission(permissions: TenantPermission[], role: UserRole = getCurrentUserRole()): boolean {
  return permissions.some((permission) => hasPermission(permission, role));
}

export function hasAnyRole(allowedRoles: UserRole[]): boolean {
  const currentRole = getCurrentUserRole();
  return allowedRoles.includes(currentRole);
}

export function getRoleCapabilities(role: UserRole = getCurrentUserRole()) {
  const can = (permission: TenantPermission) => hasPermission(permission, role);

  return {
    role,
    isAdmin: role === 'admin',
    isManager: role === 'manager',
    isStaff: role === 'staff',
    canViewReports: can(TENANT_PERMISSIONS.REPORTS_READ),
    canViewInsights: can(TENANT_PERMISSIONS.INSIGHTS_READ),
    canViewOperationalActionCenter: can(TENANT_PERMISSIONS.OPERATIONAL_ACTION_CENTER_READ),
    canViewPlatformReliability: can(TENANT_PERMISSIONS.PLATFORM_RELIABILITY_READ),
    canViewDecisionIntelligence: can(TENANT_PERMISSIONS.DECISION_INTELLIGENCE_READ),
    canGovernDecisionIntelligence: can(TENANT_PERMISSIONS.DECISION_INTELLIGENCE_GOVERN),
    canViewUsers: can(TENANT_PERMISSIONS.USERS_READ),
    canManageUsers: can(TENANT_PERMISSIONS.USERS_WRITE),
    canViewRolePermissions: can(TENANT_PERMISSIONS.ROLE_PERMISSIONS_READ),
    canManageRolePermissions: can(TENANT_PERMISSIONS.ROLE_PERMISSIONS_WRITE),
    canViewAdminSystem: can(TENANT_PERMISSIONS.SYSTEM_STATUS_READ),
    canViewSystemContext: can(TENANT_PERMISSIONS.SYSTEM_CONTEXT_READ),
    canManageProducts: can(TENANT_PERMISSIONS.PRODUCTS_WRITE),
    canViewProductPackages: can(TENANT_PERMISSIONS.PRODUCT_PACKAGES_READ),
    canManageProductPackages: can(TENANT_PERMISSIONS.PRODUCT_PACKAGES_WRITE),
    canViewSuppliers: can(TENANT_PERMISSIONS.SUPPLIERS_READ),
    canManageSuppliers: can(TENANT_PERMISSIONS.SUPPLIERS_WRITE),
    canManageStorageLocations: can(TENANT_PERMISSIONS.STORAGE_LOCATIONS_WRITE),
    canViewStock: can(TENANT_PERMISSIONS.STOCK_READ),
    canManageAlerts: can(TENANT_PERMISSIONS.ALERTS_WRITE),
    canOverrideAlerts: can(TENANT_PERMISSIONS.ALERTS_OVERRIDE),
    canManageShipments: can(TENANT_PERMISSIONS.SHIPMENTS_WRITE),
    canSendShipments: can(TENANT_PERMISSIONS.SHIPMENTS_SEND),
    canReceiveShipments: can(TENANT_PERMISSIONS.SHIPMENTS_RECEIVE),
    canFinalizeShipments: can(TENANT_PERMISSIONS.SHIPMENTS_FINALIZE),
    canAutoReorderShipments: can(TENANT_PERMISSIONS.SHIPMENTS_AUTO_REORDER),
    canViewShipmentItems: can(TENANT_PERMISSIONS.SHIPMENT_ITEMS_READ),
    canManageShipmentItems: can(TENANT_PERMISSIONS.SHIPMENT_ITEMS_WRITE),
    canConsumeStock: can(TENANT_PERMISSIONS.STOCK_CONSUME),
    canCountStock: can(TENANT_PERMISSIONS.STOCK_COUNT),
    canAdjustStock: can(TENANT_PERMISSIONS.STOCK_ADJUST),
    canViewInventoryUsage: can(TENANT_PERMISSIONS.INVENTORY_USAGE_READ),
    canRecordInventoryUsage: can(TENANT_PERMISSIONS.INVENTORY_USAGE_RECORD),
    canBulkRecordInventoryUsage: can(TENANT_PERMISSIONS.INVENTORY_USAGE_BULK_RECORD),
    canReverseInventoryUsage: can(TENANT_PERMISSIONS.INVENTORY_USAGE_REVERSE),
    canReviewInventoryUsage: can(TENANT_PERMISSIONS.INVENTORY_USAGE_REVIEW),
    canCloseInventoryUsagePeriods: can(TENANT_PERMISSIONS.INVENTORY_USAGE_CLOSE_PERIOD),
    canManageInventoryUsageTemplates: can(TENANT_PERMISSIONS.INVENTORY_USAGE_MANAGE_TEMPLATES),
    canRunScheduledInventoryUsage:
      can(TENANT_PERMISSIONS.INVENTORY_USAGE_RUN_SCHEDULED) &&
      can(TENANT_PERMISSIONS.STOCK_CONSUME) &&
      can(TENANT_PERMISSIONS.INVENTORY_USAGE_RECORD),
    canViewInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_READ),
    canCreateInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CREATE),
    canSubmitInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_SUBMIT),
    canApproveInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_APPROVE),
    canFulfillInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_FULFILL),
    canCancelOwnInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_OWN),
    canCancelAnyInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_ANY),
    canCancelInventoryRequisitions: can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_OWN) || can(TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_ANY),
    canViewInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_READ),
    canCreateInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CREATE),
    canAllocateInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_ALLOCATE),
    canFulfillInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_FULFILL),
    canReleaseInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_RELEASE),
    canCancelOwnInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_OWN),
    canCancelAnyInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_ANY),
    canCancelInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_OWN) || can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_CANCEL_ANY),
    canExpireInventoryReservations: can(TENANT_PERMISSIONS.INVENTORY_RESERVATIONS_EXPIRE),
    canViewStockTransfers: can(TENANT_PERMISSIONS.STOCK_TRANSFERS_READ),
    canCreateStockTransfers: can(TENANT_PERMISSIONS.STOCK_TRANSFERS_CREATE),
    canUpdateStockTransfers: can(TENANT_PERMISSIONS.STOCK_TRANSFERS_UPDATE),
    canExecuteStockTransfers: can(TENANT_PERMISSIONS.STOCK_TRANSFERS_EXECUTE),
    canCancelStockTransfers: can(TENANT_PERMISSIONS.STOCK_TRANSFERS_CANCEL),
    canViewPurchaseOrders: can(TENANT_PERMISSIONS.PURCHASE_ORDERS_READ),
    canCreatePurchaseOrders: can(TENANT_PERMISSIONS.PURCHASE_ORDERS_CREATE),
    canUpdatePurchaseOrders: can(TENANT_PERMISSIONS.PURCHASE_ORDERS_UPDATE),
    canSubmitPurchaseOrders: can(TENANT_PERMISSIONS.PURCHASE_ORDERS_SUBMIT),
    canApprovePurchaseOrders: can(TENANT_PERMISSIONS.PURCHASE_ORDERS_APPROVE),
    canCancelPurchaseOrders: can(TENANT_PERMISSIONS.PURCHASE_ORDERS_CANCEL),
    canReadTenantSettings: can(TENANT_PERMISSIONS.TENANT_READ),
    canUpdateTenantSettings: can(TENANT_PERMISSIONS.TENANT_UPDATE),
    canDeleteTenantSettings: can(TENANT_PERMISSIONS.TENANT_DELETE),
    canViewExecutionRequests: can(TENANT_PERMISSIONS.EXECUTION_REQUESTS_VIEW),
    canCreateExecutionRequests: can(TENANT_PERMISSIONS.EXECUTION_REQUESTS_CREATE),
    canSubmitExecutionRequests: can(TENANT_PERMISSIONS.EXECUTION_REQUESTS_SUBMIT),
    canCancelExecutionRequests: can(TENANT_PERMISSIONS.EXECUTION_REQUESTS_CANCEL),
    canReviewExecutionRequests: can(TENANT_PERMISSIONS.EXECUTION_REQUESTS_REVIEW),
    canExecuteExecutionRequests: can(TENANT_PERMISSIONS.EXECUTION_REQUESTS_EXECUTE),
    canViewAutomationSchedules: can(TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_VIEW),
    canCreateAutomationSchedules: can(TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_CREATE),
    canUpdateAutomationSchedules: can(TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_UPDATE),
    canPauseAutomationSchedules: can(TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_PAUSE),
    canResumeAutomationSchedules: can(TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_RESUME),
    canDisableAutomationSchedules: can(TENANT_PERMISSIONS.AUTOMATION_SCHEDULES_DISABLE),
    canViewSessions: role === 'admin' || role === 'manager' || role === 'staff',
    canViewTenantDiagnostics: can(TENANT_PERMISSIONS.TENANT_DIAGNOSTICS_READ),
    canCreateInventoryOptimization: can(TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_CREATE),
    canGovernInventoryOptimization: can(TENANT_PERMISSIONS.INVENTORY_OPTIMIZATION_GOVERN),
    canViewAudit: can(TENANT_PERMISSIONS.AUDIT_READ)
  };
}
