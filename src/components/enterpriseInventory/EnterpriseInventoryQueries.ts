...
export function useEnterpriseInventoryQueries({
  ...
  const productCostActionSupplierSummaryQuery = useQuery({ queryKey: ['enterprise-product-cost-action-supplier-summary'], queryFn: fetchProductCostActionSupplierSummary , enabled: canReadProducts && canReadSuppliers && tabIs('cost-control') });
