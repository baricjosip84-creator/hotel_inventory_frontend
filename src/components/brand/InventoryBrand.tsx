...
type Props = { compact?: boolean; tone?: 'light' | 'dark'; style?: CSSProperties };

export function InventoryMark({ size = 36, tone = 'light' }: { size?: number; tone?: 'light' | 'dark' }) {
  const stroke = tone === 'dark' ? '#dbeafe' : '#2563eb';
  const accent = tone === 'dark' ? '#60a5fa' : '#1d4ed8';
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden="true" focusable="false">
      ...
      <path d="M6.8 14.4 24 24.1l17.2-9.7M24 24.1v19.1" fill="none" stroke={accent} strokeWidth="3.4" strokeLinejoin="round" strokeLinecap="round" />
      <path d="m15.2 18.9 8.8-5 8.8 5-8.8 5-8.8-5Z" fill={inner} opacity="0.96" />
    </svg>
  );
}

export function InventoryBrand({ compact = false, tone = 'light', style }: Props) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: compact ? 10 : 12, minWidth: 0, ...style }}>
      <InventoryMark size={compact ? 34 : 42} tone={tone} />
