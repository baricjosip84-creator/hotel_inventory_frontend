...
const box = { background: '#fff', border: '1px solid #dbe4ef', borderRadius: 12, padding: 18, marginBottom: 16, boxShadow: '0 1px 2px rgba(15, 23, 42, 0.04)' } as const;
const input = { padding: '10px 12px', minHeight: 40, border: '1px solid #cbd5e1', borderRadius: 10, width: '100%', boxSizing: 'border-box', background: '#fff', color: '#0f172a' } as const;
const button = { padding: '9px 12px', minHeight: 38, borderRadius: 10, border: '1px solid #cbd5e1', cursor: 'pointer', background: '#fff', color: '#0f172a', fontWeight: 700 } as const;
const primaryButton = { ...button, border: '1px solid #1d4ed8', background: '#2563eb', color: '#fff' } as const;
const dangerButton = { ...button, border: '1px solid #fecaca', background: '#fff1f2', color: '#b91c1c' } as const;
  ...
  return <div style={{ display: 'grid', gap: 16 }}>
    ...
    {(canCreate || editingOrder) ? <section style={box}>
      ...
      {orderForm.items.map((line, index) => <div key={index} style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr auto', gap: 8, marginTop: 10 }}>
        ...
        <button style={dangerButton} disabled={orderForm.items.length === 1} onClick={() => setOrderForm({ ...orderForm, items: orderForm.items.filter((_, i) => i !== index) })}>Remove</button>
      </div>)}
      <div style={{ display: 'flex', gap: 8, marginTop: 10 }}><button style={button} onClick={() => setOrderForm({ ...orderForm, items: [...orderForm.items, { product_id: '', storage_location_id: '', quantity: '1', uom_code: '' }] })}>Add line</button><button style={primaryButton} disabled={!orderForm.customer_id || cleanOrderItems(orderForm).length === 0 || mutation.isPending} onClick={saveOrder}>{editingOrder ? 'Save Draft' : 'Create Order'}</button>{editingOrder ? <button style={button} onClick={() => { setEditingOrder(null); setOrderForm(emptyOrder); }}>Cancel Edit</button> : null}</div>
    </section> : null}
      ...
      {orderRows.length === 0 ? <p>No customer orders yet.</p> : orderRows.map((order) => {
        ...
        return <div key={order.id} style={{ borderTop: '1px solid #e5e7eb', padding: '12px 0' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 9 }}>
            ...
            {order.status === 'draft' && canUpdate ? <button style={primaryButton} onClick={() => mutation.mutate({ path: `/outbound/orders/${order.id}/confirm`, version: Number(order.version) })}>Confirm & Reserve Stock</button> : null}
            {['confirmed', 'partially_dispatched'].includes(order.status) && canUpdate ? <button style={primaryButton} onClick={() => startPicking(order)}>{order.status === 'partially_dispatched' ? 'Pick Remaining' : 'Start Picking'}</button> : null}
            {order.status === 'picking' && canUpdate ? <button style={primaryButton} onClick={() => setPickOrderId(order.id)}>Open Picking</button> : null}
            {order.status === 'picking' && canUpdate && openPicked > 0 ? <button style={primaryButton} onClick={() => mutation.mutate({ path: `/outbound/orders/${order.id}/mark-packed`, version: Number(order.version) })}>Mark Picked Stock Packed</button> : null}
            {['picking', 'packed'].includes(order.status) && canUpdate && openPicked > 0 ? <button style={dangerButton} onClick={() => { if (window.confirm('Clear the current picked quantities and pick again?')) mutation.mutate({ path: `/outbound/orders/${order.id}/reset-picks`, version: Number(order.version) }); }}>Clear Picks</button> : null}
            {order.status === 'packed' && canDispatch && openPacked > 0 ? <button style={primaryButton} onClick={() => { if (window.confirm(`Dispatch ${openPacked} packed unit(s)? Stock will be removed now.`)) mutation.mutate({ path: `/outbound/orders/${order.id}/dispatch`, version: Number(order.version) }); }}>Dispatch Packed Stock</button> : null}
            {!['dispatched', 'cancelled'].includes(order.status) && canCancel ? <button style={dangerButton} onClick={() => { const reason = window.prompt(dispatched > 0 ? 'Reason for cancelling the undelivered remainder' : 'Cancellation reason'); if (reason && reason.trim().length >= 3) mutation.mutate({ path: `/outbound/orders/${order.id}/cancel`, body: { reason: reason.trim() }, version: Number(order.version) }); }}>Cancel{dispatched > 0 ? ' Remainder' : ''}</button> : null}
          </div>
        </div>;
      })}
    </section>
    ...
    {canReturnRead ? <section style={box}>
      ...
      {canReturnCreate ? <div style={{ display: 'grid', gap: 9, marginBottom: 16 }}>
        ...
        <div><button style={primaryButton} disabled={!returnForm.allocation_id || !returnForm.storage_location_id || !returnForm.reason.trim() || mutation.isPending || Boolean(selectedReturnTrace?.serial_numbers?.length && (!Number.isInteger(Number(returnForm.quantity)) || returnForm.serial_numbers.length !== Number(returnForm.quantity)))} onClick={createReturn}>Create Customer Return</button></div>
      </div> : null}
      {returnRows.length === 0 ? <p>No customer returns yet.</p> : returnRows.map((row) => <div key={row.id} style={{ borderTop: '1px solid #e5e7eb', padding: '10px 0' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}><strong>{row.return_number} · {row.customer_name} · {row.order_number}</strong><span>{row.status}</span></div><div style={{ ...muted, marginTop: 4 }}>{row.reason}</div><div style={{ marginTop: 5, fontSize: 14 }}>{row.items.map((item) => `${item.product_name} ${item.quantity} ${item.product_unit} → ${item.condition} @ ${item.storage_location_name}`).join(' · ')}</div>{row.status === 'draft' ? <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>{canReturnReceive ? <button style={primaryButton} onClick={() => { if (window.confirm('Receive this customer return into inventory now?')) mutation.mutate({ path: `/outbound/returns/${row.id}/receive`, version: Number(row.version) }); }}>Receive Return</button> : null}{canReturnCancel ? <button style={dangerButton} onClick={() => { const reason = window.prompt('Return cancellation reason'); if (reason && reason.trim().length >= 3) mutation.mutate({ path: `/outbound/returns/${row.id}/cancel`, version: Number(row.version), body: { reason: reason.trim() } }); }}>Cancel Return</button> : null}</div> : null}</div>)}
    </section> : null}
  </div>;
}
