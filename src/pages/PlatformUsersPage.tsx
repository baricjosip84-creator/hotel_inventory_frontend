...
type PlatformUser = {
  ...
  open_support_session_count?: number | null;
};
...
export default function PlatformUsersPage() {
  ...
  const canRevokeSessions = hasPlatformPermission(PLATFORM_PERMISSIONS.PLATFORM_SESSIONS_REVOKE);
    ...
    <section className="io-workspace-panel platform-users__section">
      ...
      {users.length ? <div className="platform-users__list">{users.map((user) => {
        ...
        return <article key={user.id} className="platform-users__card">
          ...
          <div className="platform-users__metrics-grid"><div><span>Active Platform sessions</span><strong>{canReadSessions ? (user.active_session_count ?? 0) : 'Restricted'}</strong></div><div><span>Open Support Sessions</span><strong>{canReadSupportSessions ? (user.open_support_session_count ?? 0) : 'Restricted'}</strong></div><div><span>Failed logins</span><strong>{user.failed_login_count ?? 0}</strong></div><div><span>Last login</span><strong>{dateTime(user.last_login_at)}</strong></div><div><span>Password changed</span><strong>{dateTime(user.password_changed_at)}</strong></div><div><span>Updated</span><strong>{dateTime(user.updated_at)}</strong></div></div>
