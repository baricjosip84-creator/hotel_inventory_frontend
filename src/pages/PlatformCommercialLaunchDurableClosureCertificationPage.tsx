...
function evidenceLinksForRow(row: DurableClosureCertificationRow): PageLink[] {
...
function badgeStyle(value: string): CSSProperties {
  ...
  if (value.includes('high') || value.includes('waiting') || value.includes('manual') || value.includes('uncertified')) return { ...styles.badge, background: '#fef3c7', color: '#92400e' };
    ...
    <div style={styles.page}>
      <section style={styles.header}>
        <div>
          ...
          <h1 style={styles.title}>Commercial Launch Durable Closure Certification</h1>
          <p style={styles.description}>
            Step 237 turns steady-state resolution verification rows into a read-only durable closure certification board.
            It requires owner signoff, sustainment acceptance, reopen thresholds, next review dates, and escalation decisions
            before the commercial launch track is treated as durably closed.
          </p>
        </div>
        <div style={styles.headerMeta}>
          <span style={badgeStyle(data?.posture || 'loading')}>{humanize(data?.posture || 'loading')}</span>
          <span style={styles.generated}>{data?.generated_at ? new Date(data.generated_at).toLocaleString() : 'Not generated yet'}</span>
          <button type="button" style={styles.button} onClick={() => durableClosureCertification.refetch()} disabled={durableClosureCertification.isFetching}>
            {durableClosureCertification.isFetching ? 'Refreshing...' : durableClosureCertification.error ? 'Retry' : 'Refresh'}
          </button>
        </div>
      </section>

      {durableClosureCertification.isLoading ? <div style={styles.card}>Loading commercial launch durable closure certification board...</div> : null}
      {durableClosureCertification.error ? <div style={styles.error}>Failed to load commercial launch durable closure certification board.</div> : null}
      ...
      {data ? (
        <>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Durable closure dependency chain</h2>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Durable closure certification rows</h2>
                ...
                <tbody>
                  {data.durable_closure_certification_rows.map((row) => (
                    <tr key={row.code}>
                      ...
                      <td style={styles.td}><span style={badgeStyle(row.durable_closure_certification_status)}>{humanize(row.durable_closure_certification_status)}</span></td>
                      <td style={styles.td}>{row.required_durable_closure_certification_evidence.join(', ')}</td>
                      <td style={styles.td}><ul style={styles.list}>{row.durable_closure_certification_controls.map((control) => <li key={control}>{control}</li>)}</ul></td>
          ...
          <section style={styles.twoColumn}>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Durable closure rules</h2>
              <ul style={styles.list}>{data.durable_closure_certification_rules.map((rule) => <li key={rule}>{rule}</li>)}</ul>
            </article>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Limitations</h2>
              <ul style={styles.list}>{data.durable_closure_certification_limitations.map((item) => <li key={item}>{item}</li>)}</ul>
            </article>
          </section>
