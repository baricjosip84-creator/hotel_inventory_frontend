...
function evidenceLinksForRow(row: TransitionRow): PageLink[] {
  const links: PageLink[] = [
    { label: 'Growth Observation', to: '/platform/commercial-launch-additional-growth-observation' },
    { label: 'Additional Growth Authorization', to: '/platform/commercial-launch-additional-growth-authorization' }
  ];

  if (row.domain.includes('growth_exit')) {
  ...
  if (row.domain.includes('operating_cadence')) {
    ...
    links.push({ label: 'Backup Restore', to: '/platform/backup-restore' });
    links.push({ label: 'Deployment Validation', to: '/platform/deployment-validation' });
  }
  if (row.domain.includes('support_handover')) {
  ...
  if (row.domain.includes('monitoring_ownership')) {
  ...
  if (row.domain.includes('executive_acceptance')) {
    ...
    <div style={styles.page}>
      <section style={styles.header}>
        <div>
          ...
          <h1 style={styles.title}>Commercial Launch Steady-State Transition</h1>
          <p style={styles.description}>
            Step 230 turns additional-growth observation into a steady-state operations transition board. It tracks growth exit,
            operating cadence, support handover, customer-success handover, billing entitlement reconciliation, monitoring ownership,
            incident prevention, and executive acceptance without changing tenants, billing, feature flags, support records, incidents, or rollout scope.
          </p>
        </div>
        <div style={styles.headerMeta}>
          <span style={badgeStyle(data?.posture || 'loading')}>{humanize(data?.posture || 'loading')}</span>
          <span style={styles.generated}>{data?.generated_at ? new Date(data.generated_at).toLocaleString() : 'Not generated yet'}</span>
          <button type="button" style={styles.button} onClick={() => transition.refetch()} disabled={transition.isFetching}>
            {transition.isFetching ? 'Refreshing...' : transition.error ? 'Retry' : 'Refresh'}
          </button>
        </div>
      </section>

      {transition.isLoading ? <div style={styles.card}>Loading commercial launch steady-state transition board...</div> : null}
      {transition.error ? <div style={styles.error}>Failed to load commercial launch steady-state transition board.</div> : null}
      ...
      {data ? (
        <>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Transition chain</h2>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Transition rows</h2>
                ...
                <tbody>
                  {data.transition_rows.map((row) => (
                    <tr key={row.code}>
                      ...
                      <td style={styles.td}><span style={badgeStyle(row.transition_status)}>{humanize(row.transition_status)}</span></td>
                      <td style={styles.td}>{row.required_transition_evidence.join(', ')}</td>
                      <td style={styles.td}>
                        <ul style={styles.list}>
                          {row.transition_controls.map((control) => <li key={control}>{control}</li>)}
                        </ul>
                      </td>
          ...
          <section style={styles.twoColumn}>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Transition rules</h2>
              <ul style={styles.list}>{data.transition_rules.map((rule) => <li key={rule}>{rule}</li>)}</ul>
            </article>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Limitations</h2>
              <ul style={styles.list}>{data.transition_limitations.map((item) => <li key={item}>{item}</li>)}</ul>
            </article>
          </section>
