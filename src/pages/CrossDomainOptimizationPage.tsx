  ...
  return (
    <main className="decision-intelligence-page cross-domain-page io-operational-page io-workspace-page io-workspace-legacy-normalized" data-cross-domain-optimization-refined="true">
      ...
      <OperationalWorkspaceHero
        ...
        description={ui('Use this page when the company has one problem but several possible ways to solve it. Record what matters, compare the possible solutions, and choose one later. Nothing on this page changes stock or places orders by itself.')}
        aside={<><OperationalWorkspaceStatus value={label(data?.governance?.cross_domain_optimization_posture, ui)} label={ui('Planning review posture · refreshed {time}').replace('{time}', lastRefreshed)} /><button className="button button--secondary" type="button" onClick={() => void refetch()} disabled={isFetching}>{isFetching ? ui('Refreshing…') : ui('Refresh evidence')}</button>{canGovern ? <button className="button" type="button" onClick={() => { if (showCreate) { setShowCreate(false); } else { setCreateStep(1); setShowCreate(true); } }}>{ui('Create decision comparison')}</button> : null}</>}
      />
      ...
      <OperationalWorkspaceStats ariaLabel={ui('Cross-domain optimization evidence summary')}>
        ...
        <OperationalWorkspaceStatCard label={ui('Solutions')} value={formatLocalizedNumber(optionCount, locale)} iconPath="/workflow-composer" tone="blue" />
      ...
      <OperationalWorkspaceTabs ariaLabel={ui('Cross-domain optimization page views')}>
        ...
        <OperationalWorkspaceTab active={view === 'plan'} iconPath="/workflow-composer" label={ui('Chosen solution')} disabled={!hasEvidence} title={!hasEvidence ? ui('Create a decision comparison first.') : undefined} onClick={() => setView('plan')} />
        ...
        <section className="card cross-domain-section cross-domain-create cross-domain-create--wizard">
          <div className="card__header cross-domain-create-header">
            <div>
              ...
              <h2>{ui('Compare possible solutions to one business problem')}</h2>
              <p className="card__subtext">{ui('This is a planning exercise only. You are describing a problem and possible solutions so they can be compared. Creating it does not change stock, create a purchase order, move inventory, or carry out a solution.')}</p>
            </div>
          ...
          {createStep === 2 ? (
            <section className="cross-domain-wizard-step">
              <div className="cross-domain-wizard-step__intro">
                ...
                <div><h3>{ui('What must a good solution achieve?')}</h3><p>{ui('These are the things management will use to judge the possible solutions. This page is most useful when two important things compete with each other.')}</p></div>
              </div>
              <div className="cross-domain-wizard-callout"><strong>{ui('Example')}</strong><span>{ui('Priority 1: reduce shortages. Priority 2: do not tie up too much money in extra stock.')}</span></div>
          ...
          {createStep === 3 ? (
            <section className="cross-domain-wizard-step">
              <div className="cross-domain-wizard-step__intro">
                ...
                <div><h3>{ui('What could the company actually do?')}</h3><p>{ui('Add the realistic choices management is considering. You are not choosing one yet. You are only describing the alternatives so they can be compared.')}</p></div>
              </div>
              <div className="cross-domain-wizard-callout"><strong>{ui('Example')}</strong><span>{ui('Solution 1: keep more stock. Solution 2: order more often. Solution 3: move stock between locations before buying.')}</span></div>
              <div className="cross-domain-builder-list cross-domain-builder-list--guided">
                {reviewDraft.options.map((option, optionIndex) => <article className="cross-domain-builder-card cross-domain-builder-card--guided cross-domain-builder-card--solution" key={`wizard-option-${optionIndex}`}>
                  <div className="cross-domain-builder-card__heading"><div><span className="cross-domain-card-kicker">{ui('Solution {number}').replace('{number}', formatLocalizedNumber(optionIndex + 1, locale))}</span><strong>{option.title.trim() || ui('Unnamed solution')}</strong></div>{reviewDraft.options.length > 1 ? <button className="button button--secondary cross-domain-remove-button" type="button" onClick={() => setReviewDraft((current) => ({ ...current, options: current.options.filter((_, itemIndex) => itemIndex !== optionIndex) }))}>{ui('Remove')}</button> : null}</div>
                  <div className="cross-domain-form-grid cross-domain-form-grid--guided">
                    <label className="cross-domain-span-2"><span className="form-label">{ui('Name this possible solution')}</span><input className="input" value={option.title} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, title: event.target.value } : item) }))} placeholder={ui('Example: Increase minimum stock for frequently short products.')} /></label>
                    <label className="cross-domain-span-2"><span className="form-label">{ui('What would the company actually do?')}</span><span className="cross-domain-field-help">{ui('Describe the action people would take if this solution is chosen.')}</span><textarea className="input" rows={2} value={option.summary} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, summary: event.target.value } : item) }))} placeholder={ui('Example: Raise minimum stock only for products that repeatedly run short.')} /></label>
                    <label className="cross-domain-span-2"><span className="form-label">{ui('What do you expect to happen?')}</span><span className="cross-domain-field-help">{ui('Describe the expected business result, not a technical score.')}</span><input className="input" value={option.projected_outcome} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, projected_outcome: event.target.value } : item) }))} placeholder={ui('Example: Fewer emergency purchases, with somewhat more stock held.')} /></label>
                  </div>
                  <div className="cross-domain-builder-heading cross-domain-builder-heading--small cross-domain-downside-heading"><div><strong>{ui('Possible downside')}</strong><p>{ui('Only add this if the solution could make something else worse, slower, more expensive, or harder.')}</p></div><button className="button button--secondary" type="button" onClick={() => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, tradeoffs: [...item.tradeoffs, emptyTradeoff()] } : item) }))}>{ui('Add a downside')}</button></div>
                  {!option.tradeoffs.length ? <p className="cross-domain-empty-note">{ui('No downside has been recorded for this solution.')}</p> : null}
                  {option.tradeoffs.map((tradeoff, tradeoffIndex) => <div className="cross-domain-tradeoff-draft cross-domain-tradeoff-draft--guided" key={`wizard-tradeoff-${tradeoffIndex}`}>
                    <div className="cross-domain-tradeoff-draft__heading"><strong>{ui('Downside {number}').replace('{number}', formatLocalizedNumber(tradeoffIndex + 1, locale))}</strong><button className="button button--secondary cross-domain-remove-button" type="button" onClick={() => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, tradeoffs: item.tradeoffs.filter((_, partIndex) => partIndex !== tradeoffIndex) } : item) }))}>{ui('Remove')}</button></div>
                    <label><span className="form-label">{ui('What could get worse?')}</span><input className="input" value={tradeoff.explanation} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, tradeoffs: item.tradeoffs.map((part, partIndex) => partIndex === tradeoffIndex ? { ...part, explanation: event.target.value } : part) } : item) }))} placeholder={ui('Example: More money will be tied up in inventory.')} /></label>
                    <details className="cross-domain-advanced"><summary>{ui('How the app classifies this downside')}</summary><div className="cross-domain-form-grid cross-domain-advanced-grid"><label><span className="form-label">{ui('Priority affected')}</span><select className="input" value={tradeoff.objective_type} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, tradeoffs: item.tradeoffs.map((part, partIndex) => partIndex === tradeoffIndex ? { ...part, objective_type: event.target.value } : part) } : item) }))}>{OBJECTIVE_TYPE_OPTIONS.map((value) => <option key={value} value={value}>{label(value, ui)}</option>)}</select></label><label><span className="form-label">{ui('Business area')}</span><select className="input" value={tradeoff.tradeoff_domain} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, tradeoffs: item.tradeoffs.map((part, partIndex) => partIndex === tradeoffIndex ? { ...part, tradeoff_domain: event.target.value } : part) } : item) }))}>{OBJECTIVE_DOMAIN_OPTIONS.map((value) => <option key={value} value={value}>{label(value, ui)}</option>)}</select></label><label><span className="form-label">{ui('Overall effect')}</span><select className="input" value={tradeoff.impact_direction} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, tradeoffs: item.tradeoffs.map((part, partIndex) => partIndex === tradeoffIndex ? { ...part, impact_direction: event.target.value } : part) } : item) }))}>{IMPACT_DIRECTION_OPTIONS.map((value) => <option key={value} value={value}>{label(value, ui)}</option>)}</select></label><label><span className="form-label">{ui('Impact score')}</span><input className="input" type="number" min="-1" max="1" step="0.01" value={tradeoff.impact_score} onChange={(event) => setReviewDraft((current) => ({ ...current, options: current.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, tradeoffs: item.tradeoffs.map((part, partIndex) => partIndex === tradeoffIndex ? { ...part, impact_score: event.target.value } : part) } : item) }))} /></label></div></details>
                  </div>)}
              ...
              <button className="button button--secondary cross-domain-add-wide" type="button" onClick={() => setReviewDraft((current) => ({ ...current, options: [...current.options, emptyOption()] }))}>{ui('Add another solution')}</button>
          ...
          {createStep === 4 ? (
            <section className="cross-domain-wizard-step">
              ...
              <div className="cross-domain-review-box"><div className="cross-domain-review-box__heading"><div><span>{ui('Possible solutions')}</span><strong>{ui('{count} solutions').replace('{count}', formatLocalizedNumber(reviewDraft.options.length, locale))}</strong></div><button className="button button--secondary" type="button" onClick={() => setCreateStep(3)}>{ui('Edit')}</button></div><div className="cross-domain-review-list">{reviewDraft.options.map((option, index) => <div key={`review-option-${index}`}><strong>{option.title || `${ui('Solution')} ${formatLocalizedNumber(index + 1, locale)}`}</strong>{option.summary ? <span>{option.summary}</span> : null}{option.projected_outcome ? <span><b>{ui('Expected')}:</b> {option.projected_outcome}</span> : null}{option.tradeoffs.filter((tradeoff) => tradeoff.explanation.trim()).map((tradeoff, tradeoffIndex) => <span key={`review-downside-${tradeoffIndex}`}><b>{ui('Possible downside')}:</b> {tradeoff.explanation}</span>)}</div>)}</div></div>
              ...
              <div className="cross-domain-create-footer"><div><strong>{ui('What happens next?')}</strong><p>{ui('The comparison will be saved. You can then open it, compare the solutions, and choose one for review. Nothing is executed automatically.')}</p></div><button className="button" type="button" disabled={createReview.isPending || !reviewDraft.title.trim() || reviewDraft.options.some((option) => !option.title.trim())} onClick={() => createReview.mutate()}>{createReview.isPending ? ui('Creating…') : ui('Save decision comparison')}</button></div>
      ...
      {!hasEvidence && !showCreate ? <section className="card cross-domain-section cross-domain-first-use"><div><span className="cross-domain-eyebrow">{ui('Start here')}</span><h2>{ui('No decision comparisons yet')}</h2><p>{canGovern ? ui('Create the first comparison when the company has a problem with more than one possible solution. The page will guide you through the problem, what matters, and the possible solutions.') : ui('No decision comparisons are available for this tenant and filter set.')}</p></div>{canGovern ? <button className="button" type="button" onClick={() => { setCreateStep(1); setShowCreate(true); }}>{ui('Create decision comparison')}</button> : null}</section> : null}
