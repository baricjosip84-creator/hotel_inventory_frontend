...
import { showTenantActionSuccess } from '../lib/actionFeedback';
import {
...
const CANONICAL_LABELS: Record<string, string> = {
  ...
  minimize: 'Lower is better', maximize: 'Higher is better', balance: 'Keep balanced', stabilize: 'Keep stable', generated: 'Generated', ranked: 'Ranked', selected_action: 'Selected action', superseded: 'Superseded', positive: 'Better', negative: 'Worse', neutral: 'No meaningful change', mixed: 'Mixed effect', observed: 'Observed', value_confirmed: 'Value confirmed', value_missed: 'Value missed', tradeoff_drift_detected: 'Tradeoff drift detected',
...
function isSourceBackedObjective(objective: OptimizationObjective): boolean {
  return Boolean(simpleRecord(objective.target_reference).source_backed);
}

function sourceBackedObjectiveTarget(ui: (key: string) => string): string {
  return ui('Structured application recommendation');
}

function projectedSourcePlanType(projected: Record<string, unknown>): string {
  return String(projected.source_plan_type || projected.source_item_type || '').trim();
}
...
function projectedSourceEvidence(projected: Record<string, unknown>, locale: AppLocale, ui: (key: string) => string): string[] {
  const impact = simpleRecord(projected.impact_snapshot);
  const number = (key: string) => formatLocalizedNumber(Number(impact[key] || 0), locale);
  const planType = projectedSourcePlanType(projected);
  if (planType === 'replenishment_optimization') return [
    `${ui('Current stock')}: ${number('current_quantity')}`,
    `${ui('Target/par stock')}: ${number('par_quantity')}`,
    `${ui('Shortage')}: ${number('shortage_to_par_quantity')}`,
    `${ui('Available elsewhere')}: ${number('alternate_facility_surplus_quantity')}`
  ];
  if (planType === 'bottleneck_detection') return [
    `${ui('Open tasks')}: ${number('task_count')}`,
    `${ui('Blocked')}: ${number('blocked_count')}`,
    `${ui('Overdue')}: ${number('overdue_count')}`,
    `${ui('Unassigned')}: ${number('unassigned_count')}`
  ];
  if (planType === 'labor_forecast') return [
    `${ui('Open tasks')}: ${number('open_task_count')}`,
    `${ui('Expected hours per day')}: ${number('daily_demand_hours')}`
  ];
  if (planType === 'facility_balancing') return [
    `${ui('Active tasks')}: ${number('task_count')}`,
    `${ui('Blocked')}: ${number('blocked_count')}`,
    `${ui('Overdue')}: ${number('overdue_count')}`,
    `${ui('Estimated work hours')}: ${number('estimated_hours')}`
  ];
  return flattenImpactFacts(impact)
    .filter(([key]) => !/(^|\.)(id|uuid|score|confidence|strategy|source|target)(_|\.|$)/i.test(key))
    .slice(0, 4)
    .map(([key, value]) => `${sourceMetricLabel(key)}: ${String(value)}`);
}


function sourceBackedSavedOptionTitle(option: OptimizationOption, projected: Record<string, unknown>, ui: (key: string) => string): string {
  const planType = projectedSourcePlanType(projected);
  const rawTitle = String(option.title || option.option_label || '').trim();
  if (planType === 'replenishment_optimization') {
    const match = rawTitle.match(/^Optimize replenishment for (.+?) at (.+?):/i);
    if (match) return ui('Stock needs attention: {subject}').replace('{subject}', `${match[1]} — ${match[2]}`);
    return ui('Stock needs attention');
  }
  if (planType === 'bottleneck_detection') return ui('Execution work needs attention');
  if (planType === 'labor_forecast') return ui('Execution workload needs staffing review');
  if (planType === 'facility_balancing') return ui('Execution workload needs attention');
  return rawTitle || ui('Source-backed action');
}

function sourceBackedSavedOptionSummary(projected: Record<string, unknown>, ui: (key: string) => string): string {
  const impact = simpleRecord(projected.impact_snapshot);
  const planType = projectedSourcePlanType(projected);
  if (planType === 'replenishment_optimization') {
    const mode = String(impact.recommendation_mode || '').trim();
    if (mode === 'transfer' || Number(impact.transfer_quantity || 0) > 0) return ui('Move available stock from another location before buying more.');
    if (mode === 'procure' || Number(impact.procurement_quantity || 0) > 0) return ui('Buy the required stock through Replenishment Planning.');
    return ui('Review the current replenishment need in Replenishment Planning.');
  }
  if (planType === 'bottleneck_detection') return ui('Open Execution Tasks and review the work that needs attention.');
  if (planType === 'labor_forecast') return ui('Open Execution Tasks and check whether staffing can cover the expected workload.');
  if (planType === 'facility_balancing') return ui('Open Execution Tasks and review the current workload.');
  return ui('Structured application evidence is available for this recommendation.');
}
...
export default function CrossDomainOptimizationPage() {
  ...
  const createSourceBackedReview = useMutation({
    ...
    onSuccess: async (result) => {
      showTenantActionSuccess(selectedRecommendations.length > 1 ? ui('Decision comparison saved.') : ui('Decision review saved.'));
      setShowCreate(false);
      setCreateStep(1);
  ...
  const runAction = useMutation({
    mutationFn: ({ runId, body }: { runId: string; body: Record<string, unknown> }) => apiRequest(`/decision-intelligence/cross-domain-optimization/runs/${encodeURIComponent(runId)}/action`, {
      method: 'POST',
      body: JSON.stringify(body),
      skipMutationFeedback: body.action === 'request_intelligence_review'
    }),
    onSuccess: async (_result, variables) => {
      if (variables.body.action === 'request_intelligence_review') showTenantActionSuccess(ui('Sent to Intelligence Review'));
      await refetch();
    }
  });
  ...
  return (
    <main className="decision-intelligence-page cross-domain-page io-operational-page io-workspace-page io-workspace-legacy-normalized" data-cross-domain-optimization-refined="true">
      ...
      {view === 'evidence' && hasEvidence ? <>
        ...
        <EvidenceSection title={ui('Business objectives')} description={ui('The goals, targets, limits, and weights used to compare options.')} rows={(data?.objectives || []) as Array<Record<string, unknown>>} headers={['Run', 'Objective', 'Business area', 'Target', 'Constraint', 'Weight']} renderRow={(row, index) => { const objective = row as OptimizationObjective; return <tr key={objective.id || index}><td>{objective.optimization_label || ui('Linked planning run')}</td><td><strong>{label(objective.objective_type, ui)}</strong><span className="cross-domain-subtext">{label(objective.target_direction, ui)}</span></td><td>{label(objective.objective_domain, ui)}</td><td>{isSourceBackedObjective(objective) ? sourceBackedObjectiveTarget(ui) : referenceText(objective.target_reference, locale, ui)}</td><td>{isSourceBackedObjective(objective) ? '—' : referenceText(objective.constraint_reference, locale, ui)}</td><td>{isSourceBackedObjective(objective) ? '—' : (numeric(objective.weight) === null ? '—' : formatLocalizedNumber(Number(objective.weight), locale, { maximumFractionDigits: 2 }))}</td></tr>; }} />
        <EvidenceSection title={ui('Planning options')} description={ui('Saved planning choices. Source-backed choices retain their originating evidence; Cross-Domain does not invent a score when none was calculated.')} rows={(data?.options || []) as Array<Record<string, unknown>>} headers={['Run', 'Option', 'Status', 'Projected score', 'Confidence']} renderRow={(row, index) => { const option = row as OptimizationOption; const projected = simpleRecord(option.projected_outcome); const sourceBacked = Boolean(projected.source_plan_type || projected.source_item_type); return <tr key={option.id || index}><td>{option.optimization_label || ui('Linked planning run')}</td><td><strong>{sourceBacked ? sourceBackedSavedOptionTitle(option, projected, ui) : (option.title || option.option_label || ui('Planning option {number}').replace('{number}', formatLocalizedNumber(index + 1, locale)))}</strong>{sourceBacked ? <span className="cross-domain-subtext">{sourceBackedSavedOptionSummary(projected, ui)}</span> : option.summary ? <span className="cross-domain-subtext">{option.summary}</span> : null}</td><td><StatusBadge value={(data?.run_detail?.options || []).length === 1 && option.option_status === 'ranked' ? 'selected_action' : option.option_status} /></td><td>{sourceBacked ? '—' : formatPercentage(option.aggregate_score, locale)}</td><td>{sourceBacked ? '—' : formatPercentage(option.confidence_score, locale)}</td></tr>; }} />
      ...
      {view === 'plan' ? selectedRun ? <>
        ...
        <section className="card cross-domain-section"><div className="card__header"><div><h2>{ui('What this plan is trying to achieve')}</h2><p className="card__subtext">{ui('Targets and constraints are shown directly so a score is not separated from the business goal it is supposed to serve.')}</p></div></div><div className="cross-domain-objective-grid">{(data?.run_detail?.objectives || []).map((objective, index) => {
          const sourceBacked = isSourceBackedObjective(objective);
          return <article className="cross-domain-objective-card" key={objective.id || index}><strong>{sourceBacked ? ui('Operational goal') : label(objective.objective_type, ui)}</strong><span>{label(objective.objective_domain, ui)} · {label(objective.target_direction, ui)}</span><p><b>{ui('Target')}:</b> {sourceBacked ? sourceBackedObjectiveTarget(ui) : referenceText(objective.target_reference, locale, ui)}</p>{sourceBacked ? null : <><p><b>{ui('Constraint')}:</b> {referenceText(objective.constraint_reference, locale, ui)}</p><p><b>{ui('Weight')}:</b> {numeric(objective.weight) === null ? '—' : formatLocalizedNumber(Number(objective.weight), locale, { maximumFractionDigits: 2 })}</p></>}</article>;
        })}</div></section>
        ...
        <section className="card cross-domain-section">
          ...
          <div className="cross-domain-option-grid">{(data?.run_detail?.options || []).map((option, index) => {
            ...
            return <article className={`cross-domain-option-card${isSelected ? ' cross-domain-option-card--selected' : ''}`} key={option.id || index}>
              <div className="cross-domain-option-title"><div><strong>{isSourceBacked ? sourceBackedSavedOptionTitle(option, projected, ui) : (option.title || option.option_label || ui('Planning option {number}').replace('{number}', formatLocalizedNumber(index + 1, locale)))}</strong><p>{isSourceBacked ? sourceBackedSavedOptionSummary(projected, ui) : (option.summary || ui('No option summary was recorded.'))}</p></div><StatusBadge value={(data?.run_detail?.options || []).length === 1 && option.option_status === 'ranked' ? 'selected_action' : option.option_status} /></div>
              {isSourceBacked ? <>
                <div className="cross-domain-explanation"><h3>{ui('What the application found')}</h3><div className="cross-domain-source-facts cross-domain-source-facts--full">{projectedSourceEvidence(projected, locale, ui).length ? projectedSourceEvidence(projected, locale, ui).map((fact) => <span key={fact}>{fact}</span>) : <span>{ui('Structured application evidence is available for this recommendation.')}</span>}</div></div>
              </> : <>
        ...
        <section className="card cross-domain-section">{(() => {
          ...
          const selectedProjected = simpleRecord(selectedOption?.projected_outcome);
          const sourcePath = projectedSourceWorkflowPath(selectedProjected);
