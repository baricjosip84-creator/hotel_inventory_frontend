...
function evidenceLinksForRow(row: RetentionRenewalReviewRow): PageLink[] {
    ...
    <div style={styles.page}>
      <section style={styles.header}>
        <div>
          ...
          <h1 style={styles.title}>Commercial Launch Retention Renewal Review</h1>
          <p style={styles.description}>
            Step 240 turns evidence retention seal rows into a read-only retention renewal review board.
            It requires previous seal locators, retained history review, accountable owner renewal acceptance,
            and next renewal dates before durable evidence-retention governance is treated as maintained over time.
          </p>
        </div>
        <div style={styles.headerMeta}>
          <span style={badgeStyle(data?.posture || 'loading')}>{humanize(data?.posture || 'loading')}</span>
          <span style={styles.generated}>{data?.generated_at ? new Date(data.generated_at).toLocaleString() : 'Not generated yet'}</span>
          <button type="button" style={styles.button} onClick={() => retentionRenewalReview.refetch()} disabled={retentionRenewalReview.isFetching}>
            {retentionRenewalReview.isFetching ? 'Refreshing...' : retentionRenewalReview.error ? 'Retry' : 'Refresh'}
          </button>
        </div>
      </section>

      {retentionRenewalReview.isLoading ? <div style={styles.card}>Loading commercial launch retention renewal review board...</div> : null}
      {retentionRenewalReview.error ? <div style={styles.error}>Failed to load commercial launch retention renewal review board.</div> : null}
      ...
      {data ? (
        <>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Retention renewal dependency chain</h2>
            <div style={styles.chainGrid}>
              {sourcePostureLinks.map((link) => {
                const postureMap: Record<string, string> = {
                  'Evidence retention seal': data.evidence_retention_seal_posture,
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Retention renewal review rows</h2>
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    <th style={styles.th}>Renewal status</th>
                    <th style={styles.th}>Required renewal evidence</th>
                ...
                <tbody>
                  {data.retention_renewal_review_rows.map((row) => (
                    <tr key={row.code}>
                      ...
                      <td style={styles.td}><span style={badgeStyle(row.retention_renewal_review_status)}>{humanize(row.retention_renewal_review_status)}</span></td>
                      <td style={styles.td}>{row.required_retention_renewal_review.join(', ')}</td>
                      <td style={styles.td}><ul style={styles.list}>{row.retention_renewal_review_controls.map((control) => <li key={control}>{control}</li>)}</ul></td>
          ...
          <section style={styles.twoColumn}>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Retention renewal rules</h2>
              <ul style={styles.list}>{data.retention_renewal_review_rules.map((rule) => <li key={rule}>{rule}</li>)}</ul>
            </article>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Limitations</h2>
              <ul style={styles.list}>{data.retention_renewal_review_limitations.map((item) => <li key={item}>{item}</li>)}</ul>
            </article>
          </section>
