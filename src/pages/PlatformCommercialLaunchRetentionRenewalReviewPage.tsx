...
import './PlatformCommercialLaunchRetentionRenewalReviewPage.css';
...
type RetentionRenewalReviewRow = {
  code: string;
  ...
  retention_renewal_review_controls: string[];
  retention_renewal_review_status: string;
  release_condition: string;
};

type RetentionRenewalReview = {
  phase: string;
  step: string;
  posture: string;
  generated_at: string;
  summary: Record<string, number>;
  retention_renewal_review_rows: RetentionRenewalReviewRow[];
  ...
  retention_renewal_review_persistence: PersistenceBoundary;
  retention_renewal_review_rules: string[];
  retention_renewal_review_limitations: string[];
...
const summaryLabels: Record<string, string> = {
  retention_renewal_review_rows_total: 'Retention renewal review rows',
  waiting_for_external_evidence_retention_seal_confirmation: 'Blocked by external retention-seal confirmation',
  waiting_for_manual_retention_renewal_review: 'Awaiting external manual renewal review',
  critical_renewal_review_rows: 'Critical renewal templates',
  high_renewal_review_rows: 'High renewal templates',
  medium_renewal_review_rows: 'Medium renewal templates',
  retention_renewal_review_records_persisted_in_application: 'Renewal review records stored here'
};

const summaryHelpers: Record<string, string> = {
  retention_renewal_review_rows_total: 'Read-only renewal-review templates derived from Evidence Retention Seal',
  waiting_for_external_evidence_retention_seal_confirmation: 'Rows blocked until the external retention-seal record is independently confirmed',
  waiting_for_manual_retention_renewal_review: 'Rows prepared only for an external manual renewal-review record',
  critical_renewal_review_rows: 'Template severity hints only; not observed external renewal-review severity',
  high_renewal_review_rows: 'Template severity hints only; not observed external renewal-review severity',
  medium_renewal_review_rows: 'Template severity hints only; not observed external renewal-review severity',
  retention_renewal_review_records_persisted_in_application: 'Expected to remain zero because this endpoint stores no external renewal-review records or acceptance decisions'
};
...
export default function PlatformCommercialLaunchRetentionRenewalReviewPage() {
  const retentionRenewalReview = useQuery({
    queryKey: ['platform', 'commercial-launch-retention-renewal-review'],
    queryFn: () => platformApiRequest<RetentionRenewalReview>('/platform/commercial-launch-retention-renewal-review'),
    refetchOnWindowFocus: false,
    staleTime: 30_000
  });

  const data = retentionRenewalReview.data;
  const summaryEntries = useMemo(() => Object.entries(data?.summary || {}), [data?.summary]);
  const initialLoadError = retentionRenewalReview.isError && !data;
  const refreshError = retentionRenewalReview.isError && Boolean(data);
  const requestError = readableError(retentionRenewalReview.error);

  return (
    <div className="io-operational-page io-workspace-page platform-retention-renewal-review">
      <OperationalWorkspaceHero
        iconPath="/platform/commercial-launch-retention-renewal-review"
        eyebrow="Platform Commercial Launch Readiness"
        title="Commercial Launch Retention Renewal Review"
        description="Read-only renewal-review preparation after Evidence Retention Seal. It organizes previous seal locators, retained-history review, accountable-owner renewal acceptance and next renewal dates without claiming that an external retention seal or renewal-review acceptance has occurred."
        meta={<>
          <OperationalWorkspaceMetaPill>{data?.step || 'Step 240 — Commercial Launch Retention Renewal Review Board'}</OperationalWorkspaceMetaPill>
          <OperationalWorkspaceMetaPill>Retention renewal review preparation only</OperationalWorkspaceMetaPill>
          <OperationalWorkspaceMetaPill>External retention-seal record required</OperationalWorkspaceMetaPill>
        </>}
        aside={
          <div className="platform-retention-renewal-review__hero-aside">
            <OperationalWorkspaceStatus value={data ? data.summary.retention_renewal_review_rows_total ?? data.retention_renewal_review_rows.length : '—'} label="renewal rows" />
            {data ? <span className="platform-retention-renewal-review__status-badge" data-tone={badgeTone(data.posture)}>{humanize(data.posture)}</span> : null}
            <div className="platform-retention-renewal-review__refresh-block">
              <span>Last refreshed: {formatDateTime(data?.generated_at)}</span>
              <button type="button" className="app-button app-button--secondary" onClick={() => void retentionRenewalReview.refetch()} disabled={retentionRenewalReview.isFetching}>
                {retentionRenewalReview.isFetching ? 'Refreshing…' : 'Refresh'}
              </button>
            </div>
          </div>
        }
      ...
      <section className="app-panel app-panel--padded platform-retention-renewal-review__boundary-panel">
        <OperationalSectionHeader
          iconPath="/platform/commercial-launch-retention-renewal-review"
          title="External retention-renewal review boundary"
          description="This board prepares retention-renewal requirements; it cannot observe or persist the real external Evidence Retention Seal outcome or external previous-seal locators, retained-history reviews, accountable-owner renewal acceptances, next renewal dates or renewal-review acceptance decisions."
        />
        <div className="platform-retention-renewal-review__boundary-grid">
          <div className="platform-retention-renewal-review__boundary-notice">
            <strong>Retention renewal review preparation only.</strong>
            <span>Independently confirm the external Evidence Retention Seal record before treating these renewal rows as active. A zero stored-review count does not prove that no external renewal-review record exists.</span>
          </div>
          <div className="platform-retention-renewal-review__supporting-pages">
            ...
            <div className="platform-retention-renewal-review__link-row">
      ...
      {retentionRenewalReview.isLoading ? <section className="app-panel app-panel--padded">Loading Commercial Launch Retention Renewal Review…</section> : null}

      {initialLoadError ? (
        <section className="app-error-state platform-retention-renewal-review__feedback" role="alert">
          <strong>Unable to load Commercial Launch Retention Renewal Review.</strong>
          <span>{requestError}</span>
          <button type="button" className="app-button app-button--danger platform-retention-renewal-review__retry" onClick={() => void retentionRenewalReview.refetch()} disabled={retentionRenewalReview.isFetching}>
            {retentionRenewalReview.isFetching ? 'Retrying…' : 'Retry'}
          </button>
        </section>
      ) : null}

      {refreshError ? (
        <section className="app-panel app-panel--padded platform-retention-renewal-review__feedback platform-retention-renewal-review__feedback--warning" role="status">
          <strong>Refresh failed.</strong>
          <span>Showing the last successful Commercial Launch Retention Renewal Review snapshot. {requestError}</span>
          <button type="button" className="app-button app-button--secondary platform-retention-renewal-review__retry" onClick={() => void retentionRenewalReview.refetch()} disabled={retentionRenewalReview.isFetching}>
            {retentionRenewalReview.isFetching ? 'Retrying…' : 'Retry refresh'}
          </button>
        </section>
      ) : null}
      ...
      {data ? (
        <>
          <OperationalWorkspaceStats ariaLabel="Retention renewal review summary">
            {summaryEntries.map(([key, value]) => (
              <OperationalWorkspaceStatCard key={key} iconPath="/platform/commercial-launch-retention-renewal-review" label={summaryLabels[key] || humanize(key)} value={value} helper={summaryHelpers[key] || 'Read-only derived snapshot metric'} />
            ))}
          </OperationalWorkspaceStats>

          <section className="app-panel app-panel--padded platform-retention-renewal-review__context-panel">
            <OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-review" title="Current evidence chain" description="Current upstream posture and persistence context used to prepare the retention-renewal review board." />
            <div className="platform-retention-renewal-review__source-grid">
            ...
            <div className="platform-retention-renewal-review__persistence-grid">
              {[
          ...
          <section className="app-panel app-panel--padded platform-retention-renewal-review__rows-section">
            <OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-review" title="Retention renewal review rows" description="Prepared renewal-review requirements. Status and severity are preparation metadata, not observed external renewal-review outcomes." />
            {data.retention_renewal_review_rows.length === 0 ? (
              <div className="platform-retention-renewal-review__empty-state">
                <strong>No retention-renewal review rows were produced.</strong>
                <span>This is not evidence that retention sealing or renewal review occurred externally. The derived board remains blocked until source evidence can be established.</span>
              </div>
            ) : (
              <div className="platform-retention-renewal-review__row-grid">
                {data.retention_renewal_review_rows.map((row) => (
                  <article key={row.code} className="app-panel platform-retention-renewal-review__row-card">
                    <div className="platform-retention-renewal-review__row-heading">
                      <div><h3>{humanize(row.code)}</h3><span>{humanize(row.domain)} · Owner: {humanize(row.owner)}</span></div>
                      <div className="platform-retention-renewal-review__row-badges">
                        <span className="platform-retention-renewal-review__status-badge" data-tone={badgeTone(row.severity_hint)}>{humanize(row.severity_hint)} severity hint</span>
                        <span className="platform-retention-renewal-review__status-badge" data-tone={badgeTone(row.retention_renewal_review_status)}>{humanize(row.retention_renewal_review_status)}</span>
                      </div>
                    </div>
                    <div className="platform-retention-renewal-review__template-note">Severity is a template hint; not an observed external renewal-review severity. Renewal-review status is preparation metadata only; not an observed external review acceptance outcome.</div>
                    <div className="platform-retention-renewal-review__source-summary">
                    ...
                    <div className="platform-retention-renewal-review__source-rows"><strong>Source Operations Cadence statuses</strong><div className="platform-retention-renewal-review__chips">{row.source_cadence_statuses.length ? row.source_cadence_statuses.map((source) => <span key={`${source.code}-${source.status}`}>{humanize(source.code)} · {humanize(source.status)} · {humanize(source.cadence)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-review__source-rows"><strong>Source Steady-state Transition row references</strong><div className="platform-retention-renewal-review__chips">{row.source_transition_rows.length ? row.source_transition_rows.map((value) => <span key={value}>{humanize(value)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-review__source-rows"><strong>Source Additional Growth Observation row references</strong><div className="platform-retention-renewal-review__chips">{row.source_observation_rows.length ? row.source_observation_rows.map((value) => <span key={value}>{humanize(value)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-review__source-rows"><strong>Source Additional Growth Authorization row references</strong><div className="platform-retention-renewal-review__chips">{row.source_authorization_rows.length ? row.source_authorization_rows.map((value) => <span key={value}>{humanize(value)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-review__field-groups">
                      <div><strong>Required source recurrence-audit evidence</strong><div className="platform-retention-renewal-review__chips">{row.required_recurrence_audit_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required source recurrence-resolution evidence</strong><div className="platform-retention-renewal-review__chips">{row.required_recurrence_resolution_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required source resolution-verification evidence</strong><div className="platform-retention-renewal-review__chips">{row.required_resolution_verification_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required source durable-closure certification evidence</strong><div className="platform-retention-renewal-review__chips">{row.required_durable_closure_certification_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required external final-archive evidence</strong><div className="platform-retention-renewal-review__chips">{row.required_final_evidence_archive.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required external retention-seal evidence</strong><div className="platform-retention-renewal-review__chips">{row.required_evidence_retention_seal.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required external retention-renewal review evidence</strong><div className="platform-retention-renewal-review__chips">{row.required_retention_renewal_review.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Retention renewal review controls</strong><ul>{row.retention_renewal_review_controls.map((control) => <li key={control}>{control}</li>)}</ul></div>
                    </div>
                    <div className="platform-retention-renewal-review__row-actions">
          ...
          <section className="platform-retention-renewal-review__rules-grid">
            <div className="app-panel app-panel--padded"><OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-review" title="Retention renewal review rules" description="Conditions that keep renewal review manual, externally evidenced and review-gated." /><ul>{data.retention_renewal_review_rules.map((rule) => <li key={rule}>{rule}</li>)}</ul></div>
            <div className="app-panel app-panel--padded"><OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-review" title="Limitations" description="What this read-only renewal-review board does not observe, certify, persist or execute." /><ul>{data.retention_renewal_review_limitations.map((item) => <li key={item}>{item}</li>)}</ul></div>
          </section>

          <section className="app-panel app-panel--padded platform-retention-renewal-review__next-step"><strong>Next operator step</strong><span>{data.next_best_step}</span></section>
          <section className="app-panel app-panel--padded platform-retention-renewal-review__snapshot-note"><strong>Snapshot interpretation</strong><span>{data.validation_note}</span><small>Generated {formatDateTime(data.generated_at)} · {data.phase}</small></section>
        </>
      ) : null}
    </div>
  );
