...
function evidenceLinksForRow(row: RecurrenceResolutionRow): PageLink[] {
    ...
    <div style={styles.page}>
      <section style={styles.header}>
        <div>
          ...
          <h1 style={styles.title}>Commercial Launch Steady-State Recurrence Resolution</h1>
          <p style={styles.description}>
            Step 235 turns recurrence audit rows into a read-only recurrence resolution board. It requires
        ...
        <div style={styles.headerMeta}>
          ...
          <button type="button" style={styles.button} onClick={() => recurrenceResolution.refetch()} disabled={recurrenceResolution.isFetching}>
            {recurrenceResolution.isFetching ? 'Refreshing...' : recurrenceResolution.error ? 'Retry' : 'Refresh'}
          </button>
        </div>
      </section>

      {recurrenceResolution.isLoading ? <div style={styles.card}>Loading commercial launch steady-state recurrence resolution board...</div> : null}
      {recurrenceResolution.error ? <div style={styles.error}>Failed to load commercial launch steady-state recurrence resolution board.</div> : null}
      ...
      {data ? (
        <>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Recurrence resolution dependency chain</h2>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Recurrence resolution rows</h2>
                ...
                <tbody>
                  {data.recurrence_resolution_rows.map((row) => (
                    <tr key={row.code}>
                      ...
                      <td style={styles.td}><span style={badgeStyle(row.recurrence_resolution_status)}>{humanize(row.recurrence_resolution_status)}</span></td>
                      <td style={styles.td}>{row.required_recurrence_resolution_evidence.join(', ')}</td>
                      <td style={styles.td}><ul style={styles.list}>{row.recurrence_resolution_controls.map((control) => <li key={control}>{control}</li>)}</ul></td>
          ...
          <section style={styles.twoColumn}>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Recurrence resolution rules</h2>
              <ul style={styles.list}>{data.recurrence_resolution_rules.map((rule) => <li key={rule}>{rule}</li>)}</ul>
            </article>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Limitations</h2>
              <ul style={styles.list}>{data.recurrence_resolution_limitations.map((item) => <li key={item}>{item}</li>)}</ul>
            </article>
          </section>
