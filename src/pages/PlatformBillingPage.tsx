...
export default function PlatformBillingPage() {
  ...
  const billingEvents = Array.isArray(detailsQuery.data?.events) ? detailsQuery.data.events : [];
  useEffect(() => {
  ...
  const rawData = overviewQuery.data as BillingWorkspace | BillingTenant[] | undefined;
  const legacyTenants = Array.isArray(rawData) ? rawData : null;
  const data = legacyTenants ? undefined : rawData;
  const tenants = legacyTenants || (Array.isArray(data?.tenants) ? data.tenants : []);
  const summary = data?.summary;
  const pagination = data?.pagination;
  const catalogPlans = Array.isArray(catalogQuery.data?.plans) ? catalogQuery.data.plans : [];
  const reconciliationActions = Array.isArray(reconciliationResult?.actions) ? reconciliationResult.actions : [];
  const selectedPlan = catalogPlans.find((plan) => plan.plan_code === billingForm.plan_code) || null;
  ...
  const pageEnd = pagination ? Math.min(pagination.offset + tenants.length, pagination.total) : tenants.length;
  ...
  return <div className="io-operational-page io-workspace-page platform-billing">
    ...
    <section className="io-workspace-panel platform-billing__section">
      ...
      {tenants.length ? <div className="platform-billing__table-wrap"><table className="platform-billing__table"><thead><tr><th>Tenant</th><th>Billing state</th><th>Plan</th><th>Period</th><th>Provider reference</th><th>Events</th><th>Action</th></tr></thead><tbody>{tenants.map((tenant) => <tr key={tenant.id} data-selected={tenant.id === requestedTenantId ? 'true' : 'false'}><td><strong>{tenant.name}</strong><small>{tenant.location || 'Location not recorded'} · {pretty(tenant.status)}</small></td><td><span className="platform-billing__badge" data-tone={tenant.billing_status === 'past_due' || tenant.billing_status === 'cancelled' ? 'danger' : tenant.billing_status === 'active' || tenant.billing_status === 'comped' ? 'good' : 'neutral'}>{pretty(tenant.billing_status)}</span></td><td>{tenant.plan_code || 'Not configured'}</td><td><strong>{formatDateTime(tenant.current_period_ends_at)}</strong><small>Trial {formatDateTime(tenant.trial_ends_at)}</small></td><td className="platform-billing__wrap">{tenant.billing_customer_reference || 'Not linked'}</td><td><strong>{tenant.billing_event_count || 0}</strong><small>Last {formatDateTime(tenant.last_billing_event_at)}</small></td><td><button type="button" className="app-button app-button--secondary" onClick={() => updateParams({ tenant_id: tenant.id })}>{tenant.id === requestedTenantId ? 'Selected' : 'Open'}</button></td></tr>)}</tbody></table></div> : rawData ? <div className="platform-billing__empty"><strong>No tenants matched.</strong><span>No tenant billing record matched the current filters.</span></div> : null}
      ...
      {reconciliationResult ? <div className="platform-billing__reconciliation"><strong>{reconciliationResult.dry_run ? 'Reconciliation preview' : 'Reconciliation applied'}</strong><span>Inspected {reconciliationResult.inspected_count}; issues {reconciliationResult.issue_count}; applied {reconciliationResult.applied_count}.</span>{reconciliationActions.slice(0, 12).map((action) => <small key={`${action.tenant_id}-${action.action}`}>{action.tenant_name || action.tenant_id}: {pretty(action.action)} · {action.reason}</small>)}</div> : null}
    </section>
    ...
    {requestedTenantId ? <section className="io-workspace-panel platform-billing__section">
      ...
      {detailsQuery.data && !selectedTenant ? <div className="platform-billing__warning" role="status"><strong>Selected tenant evidence is temporarily incomplete.</strong><span>The backend response did not include the tenant profile. Refresh after the backend/frontend deployment is synchronized.</span></div> : null}
      {selectedTenant ? <>
        <div className="platform-billing__selected-heading"><div><strong>{selectedTenant.name}</strong><span>{pretty(selectedTenant.billing_status)} · {selectedTenant.plan_code || 'No plan'} · {selectedTenant.billing_customer_reference || 'No provider reference'}</span></div>{canReadAudit ? <Link to={`/platform/audit?tenant_id=${encodeURIComponent(requestedTenantId)}&source=billing`}>Audit evidence</Link> : null}</div>
        ...
        <div className="platform-billing__workbench-grid">
          <article className="platform-billing__card"><h3>Billing profile</h3><p>Plan linkage, provider reference, dates and notes. Subscription status is changed separately.</p><div className="platform-billing__form-grid">
            <label>Plan<select value={billingForm.plan_code} onChange={(event) => setBillingForm({ ...billingForm, plan_code: event.target.value })}><option value="">No catalog plan</option>{catalogPlans.map((plan) => <option key={plan.plan_code} value={plan.plan_code}>{plan.plan_code} · {plan.commercial_tier}</option>)}</select></label>
