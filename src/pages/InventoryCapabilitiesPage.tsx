...
const API_SCOPE_OPTIONS = [
  ...
  { value: 'purchase_orders:submit', label: 'Submit purchase order drafts', requiredPermissions: [TENANT_PERMISSIONS.PURCHASE_ORDERS_SUBMIT] },
  ...
  { value: 'shipments:update', label: 'Update shipment drafts', requiredPermissions: [TENANT_PERMISSIONS.SHIPMENTS_WRITE] },
  { value: 'shipment_items:write', label: 'Manage shipment items', requiredPermissions: [TENANT_PERMISSIONS.SHIPMENT_ITEMS_WRITE] },
  ...
  { value: 'locations:write', label: 'Manage storage locations', requiredPermissions: [TENANT_PERMISSIONS.STORAGE_LOCATIONS_WRITE] },
  ...
  { value: 'requisitions:submit', label: 'Submit requisition drafts', requiredPermissions: [TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_SUBMIT, TENANT_PERMISSIONS.INVENTORY_REQUISITIONS_CANCEL_ANY] },
...
const toApiExpiry = (value: string) => value ? new Date(value).toISOString() : null;
...
function IntegrationsPanel({ canWrite }: { canWrite: boolean }) {
  ...
  const webhookEventLabels = new Map((webhookEventCatalog.data?.items || []).map((item) => [item.event_type, item.event_type === '*' ? ui("All listed events") : `${ui(item.group)} — ${ui(item.label)}`]));
  const displayWebhookEvent = (eventType: string) => webhookEventLabels.get(eventType) || eventType;
  ...
  const editApiClient = (client: ApiClient) => { setEditingApiClient(client); setName(client.name); setDescription(client.description || ''); setApiScopes(client.scopes || []); setApiExpiresAt(toLocalDateTimeInput(client.expires_at)); setApiAllowedIps((client.allowed_ips || []).join(', ')); setError(null); };
  const updateClient = useMutation({
    mutationFn: () => apiRequest<ApiClient>(`/inventory-capabilities/api-clients/${editingApiClient!.id}`, { method: 'PATCH', body: JSON.stringify({ description, scopes: apiScopes, expires_at: toApiExpiry(apiExpiresAt), allowed_ips: apiAllowedIps.split(',').map((value) => value.trim()).filter(Boolean) }), version: editingApiClient!.version }),
    ...
    <section className="section" style={panelStyle}>
      ...
      <div className="card">
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (editingApiClient) updateClient.mutate(); else createClient.mutate(); }} style={formGridStyle}>
          ...
          <label>{ui("Expiry date and time")}<input type="datetime-local" value={apiExpiresAt} onChange={(e) => setApiExpiresAt(e.target.value)} disabled={!canWrite} /></label>
        ...
        <div style={tableWrapStyle}>
          <table><thead><tr><th>{ui("Name")}</th><th>{ui("Prefix")}</th><th>{ui("Scopes")}</th><th>{ui("Expires")}</th><th>{ui("Allowed IP addresses")}</th><th>{ui("Created by")}</th><th>{ui("Last used")}</th><th>{ui("Status")}</th><th>{ui("Actions")}</th></tr></thead><tbody>
            {!clients.isLoading && !clients.isError && !(clients.data || []).length ? <EmptyTableRow colSpan={9} message={ui("No API keys have been created yet.")} /> : null}{(clients.data || []).map((client) => <tr key={client.id}><td>{client.name}</td><td>{client.key_prefix}</td><td>{client.scopes?.map((scope) => API_SCOPE_LABELS[scope] ? ui(API_SCOPE_LABELS[scope]) : ui("Unknown API permission")).join(', ')}</td><td>{client.expires_at ? formatDate(client.expires_at, locale) : ui("Never")}</td><td>{(client.allowed_ips || []).length ? (client.allowed_ips || []).join(', ') : '—'}</td><td>{client.created_by_user_name || '—'}</td><td>{formatDate(client.last_used_at, locale)}</td><td>{client.status === 'active' && client.expires_at && new Date(client.expires_at).getTime() <= Date.now() ? ui("Expired") : displayCanonicalLabel(client.status, ui)}</td><td>{client.status === 'active' && canWrite ? <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}><button className="button button--secondary" type="button" disabled={updateClient.isPending || rotateClient.isPending || revokeClient.isPending} onClick={() => editApiClient(client)}>{ui("Edit")}</button><button className="button button--secondary" type="button" disabled={updateClient.isPending || rotateClient.isPending || revokeClient.isPending} onClick={() => { if (window.confirm(`${ui('Rotate secret')} ${client.name}?`)) rotateClient.mutate(client); }}>{ui("Rotate secret")}</button><button className="button button--secondary" type="button" disabled={updateClient.isPending || rotateClient.isPending || revokeClient.isPending} onClick={() => { if (window.confirm(`${ui('Revoke API key')} ${client.name}? ${ui('Systems using it will stop working immediately.')}`)) revokeClient.mutate(client); }}>{revokeClient.isPending ? ui("Revoking…") : ui("Revoke")}</button></div> : null}</td></tr>)}
          </tbody></table>
        </div>
      ...
      <div className="card">
        ...
        <h3>{ui("Webhooks")}</h3>
        <p className="card__subtext">{ui("Another system can give this app an HTTPS address. Choose which inventory events it should receive. “All listed events” means every event shown in this picker, not internal audit activity.")}</p>
        ...
        <form onSubmit={(e) => { e.preventDefault(); createWebhook.mutate(); }} style={formGridStyle}>
          ...
          <fieldset style={{ gridColumn: '1 / -1', border: '1px solid #e2e8f0', borderRadius: 12, padding: 14, background: '#f8fafc' }} disabled={!canWrite || webhookEventCatalog.isLoading || webhookEventCatalog.isError}><legend>{ui("Events")}</legend><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', maxHeight: 220, overflowY: 'auto' }}>{(webhookEventCatalog.data?.items || []).map(({ event_type, group, label }) => <label key={event_type} title={event_type} style={{ display: 'flex', gap: 6, alignItems: 'center' }}><input type="checkbox" checked={webhookEvents.includes(event_type)} onChange={(event) => setWebhookEvents((current) => event.target.checked ? (event_type === '*' ? ['*'] : Array.from(new Set(current.filter((value) => value !== '*').concat(event_type)))) : current.filter((value) => value !== event_type))} />{event_type === '*' ? ui("All listed events") : <span><strong>{ui(group)}</strong> — {ui(label)}</span>}</label>)}</div></fieldset>
          <div style={{ alignSelf: 'end' }}><button className="button" disabled={!canWrite || !webhooks.isSuccess || !webhookEventCatalog.isSuccess || !webhookName.trim() || !webhookUrl.trim() || webhookEvents.length === 0 || createWebhook.isPending || loadMoreWebhooks.isPending || changeWebhookStatus.isPending || rotateWebhook.isPending}>{ui("Create webhook")}</button></div>
        </form>
        {webhookEventCatalog.isError ? <div className="form-error">{ui("Unable to load webhook event choices.")}</div> : null}
