...
function IntegrationsPanel({ canWrite }: { canWrite: boolean }) {
  ...
  const createClient = useMutation({
    ...
    onError: (err) => setError(messageFrom(err, ui("Unable to create API client.")))
  });
  const revokeClient = useMutation({
    ...
    onError: (err) => setError(messageFrom(err, ui("Unable to revoke API key.")))
  });
  const saveConnection = useMutation({
    ...
    onError: (err) => setError(messageFrom(err, ui("Unable to save connection.")))
  });
  const createWebhook = useMutation({
    ...
    onError: (err) => setError(messageFrom(err, ui("Unable to create webhook.")))
  });
  const changeWebhookStatus = useMutation({
    ...
    onError: (err) => setError(messageFrom(err, ui("Unable to update webhook.")))
  });
  const rotateWebhook = useMutation({
    ...
    onError: (err) => setError(messageFrom(err, ui("Unable to rotate webhook secret.")))
  });
  const testWebhook = useMutation({
    ...
    onError: (err) => setError(messageFrom(err, ui("Unable to queue webhook test.")))
  });
    ...
    <section className="section" style={panelStyle}>
      ...
      <div className="card">
        ...
        <form onSubmit={(e) => { e.preventDefault(); createClient.mutate(); }} style={formGridStyle}>
          ...
          <fieldset style={{ gridColumn: '1 / -1', border: '1px solid #e2e8f0', borderRadius: 12, padding: 14, background: '#f8fafc' }} disabled={!canWrite}>
            ...
            <p className="card__subtext">{ui("Starts read-only. Enable write permissions only for systems that genuinely need them.")}</p>
        ...
        <div style={tableWrapStyle}>
          <table><thead><tr><th>{ui("Name")}</th><th>{ui("Prefix")}</th><th>{ui("Scopes")}</th><th>{ui("Last used")}</th><th>{ui("Status")}</th><th /></tr></thead><tbody>
            {!(clients.data || []).length ? <EmptyTableRow colSpan={6} message={ui("No API keys have been created yet.")} /> : null}{(clients.data || []).map((client) => <tr key={client.id}><td>{client.name}</td><td>{client.key_prefix}</td><td>{client.scopes?.join(', ')}</td><td>{formatDate(client.last_used_at, locale)}</td><td>{displayCanonicalLabel(client.status, ui)}</td><td>{client.status === 'active' && canWrite ? <button className="button button--secondary" type="button" disabled={revokeClient.isPending} onClick={() => { if (window.confirm(`${ui('Revoke API key')} ${client.name}? ${ui('Systems using it will stop working immediately.')}`)) revokeClient.mutate(client.id); }}>{revokeClient.isPending ? ui("Revoking…") : ui("Revoke")}</button> : null}</td></tr>)}
          </tbody></table>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <form onSubmit={(e) => { e.preventDefault(); createWebhook.mutate(); }} style={formGridStyle}>
          ...
          <div style={{ alignSelf: 'end' }}><button className="button" disabled={!canWrite || !webhookName.trim() || !webhookUrl.trim() || !webhookEvents.trim() || createWebhook.isPending}>{ui("Create webhook")}</button></div>
        </form>
        <div style={tableWrapStyle}><table><thead><tr><th>{ui("Name")}</th><th>{ui("Destination")}</th><th>{ui("Events")}</th><th>{ui("Status")}</th><th>{ui("Actions")}</th></tr></thead><tbody>
          {!(webhooks.data || []).length ? <EmptyTableRow colSpan={5} message={ui("No webhooks are configured yet.")} /> : null}{(webhooks.data || []).map((row) => <tr key={row.id}><td>{row.display_name}</td><td style={{ maxWidth: 320, wordBreak: 'break-all' }}>{row.destination_reference}</td><td>{row.event_types.join(', ')}</td><td>{displayCanonicalLabel(row.status, ui)}</td><td><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{canWrite ? <><button className="button button--secondary" type="button" disabled={testWebhook.isPending || rotateWebhook.isPending || changeWebhookStatus.isPending} onClick={() => testWebhook.mutate(row.id)}>{testWebhook.isPending ? ui("Testing…") : ui("Test")}</button><button className="button button--secondary" type="button" disabled={testWebhook.isPending || rotateWebhook.isPending || changeWebhookStatus.isPending} onClick={() => rotateWebhook.mutate(row.id)}>{rotateWebhook.isPending ? ui("Rotating…") : ui("Rotate secret")}</button><button className="button button--secondary" type="button" disabled={testWebhook.isPending || rotateWebhook.isPending || changeWebhookStatus.isPending} onClick={() => changeWebhookStatus.mutate({ id: row.id, status: row.status === 'configured' ? 'disabled' : 'configured' })}>{changeWebhookStatus.isPending ? ui("Saving…") : row.status === 'configured' ? ui("Disable") : ui("Enable")}</button></> : null}</div></td></tr>)}
        </tbody></table></div>
        ...
        <div style={tableWrapStyle}><table><thead><tr><th>{ui("Webhook")}</th><th>{ui("Event")}</th><th>{ui("Status")}</th><th>{ui("Attempts")}</th><th>{ui("HTTP")}</th><th>{ui("Created")}</th></tr></thead><tbody>
          {!(deliveries.data || []).length ? <EmptyTableRow colSpan={6} message={ui("No webhook delivery attempts yet.")} /> : null}{(deliveries.data || []).map((row) => <tr key={row.id}><td>{row.subscription_name || row.subscription_id}</td><td>{row.event_type}</td><td>{displayCanonicalLabel(row.event_status, ui)}</td><td>{formatLocalizedNumber(row.attempt_count, locale)}</td><td>{row.response_status == null ? '—' : formatLocalizedNumber(row.response_status, locale)}</td><td>{formatDate(row.created_at, locale)}</td></tr>)}
        </tbody></table></div>
      </div>

      <div className="card">
        <div className="card__label">{ui("Known external systems")}</div>
        <h3>{ui("Connection registry")}</h3>
        ...
        <form onSubmit={(e) => { e.preventDefault(); saveConnection.mutate(); }} style={formGridStyle}>
          <label>{ui("System name")}<input value={systemName} onChange={(e) => setSystemName(e.target.value)} placeholder={ui("SAP Business One")} disabled={!canWrite} /></label>
          ...
          <div style={{ alignSelf: 'end' }}><button className="button" disabled={!canWrite || !systemName.trim() || saveConnection.isPending}>{ui("Save connection")}</button></div>
        </form>
        <div style={tableWrapStyle}><table><thead><tr><th>{ui("System")}</th><th>{ui("Type")}</th><th>{ui("Direction")}</th><th>{ui("Status")}</th></tr></thead><tbody>{!(connections.data || []).length ? <EmptyTableRow colSpan={4} message={ui("No external systems are recorded yet.")} /> : null}{(connections.data || []).map((row) => <tr key={row.id}><td>{row.system_name}</td><td>{displayCanonicalLabel(row.system_type, ui)}</td><td>{displayCanonicalLabel(row.direction, ui)}</td><td>{displayCanonicalLabel(row.status, ui)}</td></tr>)}</tbody></table></div>
      </div>
    </section>
  );
}
...
function SerialsPanel({ products, locations, canWriteTracking, canReadStock, canRegisterSerial }: { products: Product[]; locations: Location[]; canWriteTracking: boolean; canReadStock: boolean; canRegisterSerial: boolean }) {
  ...
  const saveSettings = useMutation({ mutationFn: () => apiRequest(`/inventory-capabilities/products/${productId}/tracking`, { method: 'PUT', body: JSON.stringify(settings) }), onSuccess: () => { setError(null); void qc.invalidateQueries({ queryKey: ['tracking-settings', productId] }); }, onError: (e) => setError(messageFrom(e, ui("Unable to save tracking settings."))) });
  const addSerial = useMutation({
    ...
    onError: (e) => setError(messageFrom(e, ui("Unable to register serial against existing stock.")))
  });
  ...
  return <section className="section" style={panelStyle}><div className="section__title">{ui("Serial-number tracking")}</div>
    <div className="card"><div style={formGridStyle}><ProductSelect products={products} value={productId} onChange={changeProduct} />
      <label><input type="checkbox" checked={settings.serial_tracking_enabled} onChange={(e) => setSettings({ ...settings, serial_tracking_enabled: e.target.checked, require_serial_on_receipt: e.target.checked ? settings.require_serial_on_receipt : false, require_serial_on_issue: e.target.checked ? settings.require_serial_on_issue : false })} disabled={!canWriteTracking || !productId} /> {ui("Enable serial tracking")}</label>
      <label>{ui("Uniqueness")}<select value={settings.serial_uniqueness_scope} onChange={(e) => setSettings({ ...settings, serial_uniqueness_scope: e.target.value as 'product' | 'tenant' })} disabled={!canWriteTracking || !productId || !settings.serial_tracking_enabled}><option value="product">{ui("Unique within product")}</option><option value="tenant">{ui("Unique across tenant")}</option></select></label>
      <label><input type="checkbox" checked={settings.require_serial_on_receipt} onChange={(e) => setSettings({ ...settings, require_serial_on_receipt: e.target.checked })} disabled={!canWriteTracking || !productId || !settings.serial_tracking_enabled} /> {ui("Require on receipt")}</label>
      <label><input type="checkbox" checked={settings.require_serial_on_issue} onChange={(e) => setSettings({ ...settings, require_serial_on_issue: e.target.checked })} disabled={!canWriteTracking || !productId || !settings.serial_tracking_enabled} /> {ui("Require on issue")}</label>
      <div style={{ alignSelf: 'end' }}><button className="button" type="button" disabled={!canWriteTracking || !productId || saveSettings.isPending} onClick={() => saveSettings.mutate()}>{saveSettings.isPending ? ui("Saving…") : ui("Save tracking")}</button></div>
    </div><p className="card__subtext capability-help">{ui("Receipt and issue requirements apply only while serial tracking is enabled.")}</p>{error ? <div className="form-error">{error}</div> : null}</div>
    <div className="card"><h3>{ui("Register serial for existing stock")}</h3><p className="muted">{ui("This assigns a serial identity to an item that is already on hand. Choose the exact location and inventory lot. Reservation, issue, damage, quarantine and return states are changed only by their real inventory workflows.")}</p>{!canReadStock ? <div className="form-error">{ui("Stock read permission is required to view serial inventory.")}</div> : null}{canReadStock && !canRegisterSerial ? <div className="form-error">{ui("Product write and stock adjust permissions are required to register a serial against existing inventory.")}</div> : null}<form onSubmit={(e) => { e.preventDefault(); addSerial.mutate(); }} style={formGridStyle}>
      ...
      <select value={inventoryLotId} onChange={(e) => setInventoryLotId(e.target.value)} disabled={!canRegisterSerial || !locationId || lotsQuery.isLoading}><option value="">{ui("Select inventory lot…")}</option>{eligibleRegistrationLots.map((lot) => <option key={lot.id} value={lot.id}>{lotLabel(lot)}</option>)}</select>
      <button className="button" disabled={!canRegisterSerial || !productId || !serialNumber.trim() || !locationId || !inventoryLotId || !settings.serial_tracking_enabled || addSerial.isPending}>{addSerial.isPending ? ui("Registering…") : ui("Register serial")}</button>
    </form>{lotsQuery.isError ? <div className="form-error">{messageFrom(lotsQuery.error, ui("Unable to load eligible inventory lots."))}</div> : null}{canReadStock && locationId && !lotsQuery.isLoading && !lotsQuery.isError && !eligibleRegistrationLots.length ? <div className="muted">{ui("No available on-hand lot exists at this location. Receive or count stock first; serial registration never creates quantity.")}</div> : null}</div>
    {canReadStock ? <div className="card" style={tableWrapStyle}><table><thead><tr><th>{ui("Serial")}</th><th>{ui("Product")}</th><th>{ui("Status")}</th><th>{ui("Location")}</th><th>{ui("Lot / batch")}</th><th>{ui("Updated")}</th></tr></thead><tbody>{!(serialsQuery.data || []).length ? <EmptyTableRow colSpan={6} message={ui("No serial identities match the current product selection.")} /> : null}{(serialsQuery.data || []).map((serial) => <tr key={serial.id}><td>{serial.serial_number}</td><td>{serial.product_sku} — {serial.product_name}</td><td>{displayCanonicalLabel(serial.operational_status || serial.status, ui)}</td><td>{serial.storage_location_name || '-'}</td><td>{[serial.lot_number ? `${ui('Lot')} ${serial.lot_number}` : null, serial.batch_number ? `${ui('Batch')} ${serial.batch_number}` : null].filter(Boolean).join(' · ') || '—'}</td><td>{formatDate(serial.updated_at, locale)}</td></tr>)}</tbody></table></div> : null}
  </section>;
}
...
function CustomFieldsPanel({
  ...
  const entityConfigs: Record<EntityType, { label: string; plural: string; rows: Array<{ id: string; label: string }> }> = {
    ...
    shipment: { label: ui("Shipment"), plural: 'shipments', rows: shipments.map((row) => ({ id: row.id, label: row.po_number || row.qr_code || row.id.slice(0, 8) })) },
    purchase_order: { label: ui("Purchase order"), plural: 'purchase orders', rows: purchaseOrders.map((row) => ({ id: row.id, label: row.po_number || row.id.slice(0, 8) })) }
  };
  ...
  const saveDefinition = useMutation({
    ...
    onError: (errorValue) => setError(messageFrom(errorValue, ui("Unable to save custom field.")))
  });
  ...
  const saveValues = useMutation({
    ...
    onError: (errorValue) => setError(messageFrom(errorValue, ui("Unable to save custom values.")))
  });
  ...
  return (
    <section className="section" style={panelStyle}>
      ...
      <div className="card">
        <h3>{ui("Create custom field")}</h3>
        <div style={formGridStyle}>
          <label>{ui("Field key")}<input value={fieldKey} onChange={(event) => setFieldKey(event.target.value)} placeholder="country_of_origin" /></label>
          ...
          <button className="button" type="button" disabled={!canWrite || !fieldKey.trim() || !label.trim() || (dataType === 'select' && !options.split(',').some((value) => value.trim())) || saveDefinition.isPending} onClick={() => saveDefinition.mutate()}>{saveDefinition.isPending ? ui("Saving…") : ui("Save field")}</button>
        </div>
      ...
      <div className="card">
        ...
        <button className="button" style={{ marginTop: 14 }} type="button" disabled={!canWrite || !entityId || saveValues.isPending} onClick={() => saveValues.mutate()}>{saveValues.isPending ? ui("Saving…") : ui("Save custom fields")}</button>
      </div>
      <div className="card" style={tableWrapStyle}>
        <table><thead><tr><th>{ui("Key")}</th><th>{ui("Label")}</th><th>{ui("Type")}</th><th>{ui("Required")}</th></tr></thead><tbody>{!(definitions.data || []).length ? <EmptyTableRow colSpan={4} message={ui("No custom fields are configured for this entity type.")} /> : null}{(definitions.data || []).map((definition) => <tr key={definition.id}><td>{definition.field_key}</td><td>{definition.label}</td><td>{displayCanonicalLabel(definition.data_type, ui)}</td><td>{definition.is_required ? ui("Yes") : ui("No")}</td></tr>)}</tbody></table>
      </div>
    </section>
  );
}
...
function LandedCostPanel({ shipments, canWrite }: { shipments: Shipment[]; canWrite: boolean }) {
  ...
  const previewMutation = useMutation({ mutationFn: () => apiRequest<LandedPreview>('/inventory-capabilities/landed-costs/preview', { method: 'POST', body: JSON.stringify(body()) }), onSuccess: (data) => { setPreview(data); setError(null); }, onError: (e) => setError(messageFrom(e, ui("Unable to preview landed cost."))) });
  const finalizeMutation = useMutation({ mutationFn: () => apiRequest('/inventory-capabilities/landed-costs/finalize', { method: 'POST', body: JSON.stringify(body()) }), onSuccess: () => { setPreview(null); setFreight(''); setCustoms(''); setInsurance(''); setError(null); void qc.invalidateQueries({ queryKey: ['landed-cost-history'] }); void qc.invalidateQueries({ queryKey: ['inventory-capabilities-products'] }); }, onError: (e) => setError(messageFrom(e, ui("Unable to finalize landed cost."))) });
  ...
  return <section className="section" style={panelStyle}><div className="section__title">{ui("Landed-cost allocation")}</div>
    ...
    <div className="card" style={tableWrapStyle}><h3>{ui("Finalized landed costs")}</h3><table><thead><tr><th>{ui("PO")}</th><th>{ui("Method")}</th><th>{ui("Extra cost")}</th><th>{ui("Finalized")}</th></tr></thead><tbody>{!(history.data || []).length ? <EmptyTableRow colSpan={4} message={ui("No landed costs have been finalized yet.")} /> : null}{(history.data || []).map((row) => <tr key={row.id}><td>{row.po_number || row.shipment_id.slice(0, 8)}</td><td>{displayCanonicalLabel(row.allocation_method, ui)}</td><td>{formatLocalizedCurrency(Number(row.total_extra_cost), row.currency, locale)}</td><td>{formatDate(row.finalized_at, locale)}</td></tr>)}</tbody></table></div>
  </section>;
}
...
function VariantsPanel({ products, canWrite, onChanged }: { products: Product[]; canWrite: boolean; onChanged: () => void }) {
  ...
  const create = useMutation({ mutationFn: () => apiRequest(`/inventory-capabilities/products/${parentId}/variants`, { method: 'POST', body: JSON.stringify({ sku, name, barcode: barcode || null, attributes: parseAttributes(attributes) }) }), onSuccess: () => { setSku(''); setName(''); setBarcode(''); setAttributes(''); setError(null); void qc.invalidateQueries({ queryKey: ['product-variants'] }); onChanged(); }, onError: (e) => setError(messageFrom(e, ui("Unable to create variant."))) });
  const parents = products.filter((p) => !p.parent_product_id);
  return <section className="section" style={panelStyle}><div className="section__title">{ui("Product variants")}</div><div className="card"><p className="card__subtext">{ui("A variant is created as a real product underneath a parent. That means existing stock, barcodes, shipments and movements can use it immediately.")}</p><form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} style={formGridStyle}><ProductSelect products={parents} value={parentId} onChange={setParentId} label={ui("Parent product")} /><label>{ui("Variant SKU")}<input value={sku} onChange={(e) => setSku(e.target.value)} /></label><label>{ui("Variant name")}<input value={name} onChange={(e) => setName(e.target.value)} placeholder={ui("T-shirt / Red / M")} /></label><label>{ui("Barcode")}<input value={barcode} onChange={(e) => setBarcode(e.target.value)} /></label><label>{ui("Attributes")}<input value={attributes} onChange={(e) => setAttributes(e.target.value)} placeholder={ui("Color=Red, Size=M")} /></label><button className="button" disabled={!canWrite || !parentId || !sku.trim() || !name.trim() || create.isPending}>{create.isPending ? ui("Creating…") : ui("Create variant")}</button></form>{error ? <div className="form-error">{error}</div> : null}</div><div className="card" style={tableWrapStyle}><table><thead><tr><th>{ui("Parent")}</th><th>{ui("SKU")}</th><th>{ui("Variant")}</th><th>{ui("Attributes")}</th><th>{ui("Stock")}</th></tr></thead><tbody>{!(variants.data || []).length ? <EmptyTableRow colSpan={5} message={ui("No product variants have been created yet.")} /> : null}{(variants.data || []).map((v) => <tr key={v.id}><td>{v.parent_product_name}</td><td>{v.sku}</td><td>{v.name}</td><td>{Object.entries(v.variant_attributes || {}).map(([k,val]) => `${k}: ${String(val)}`).join(', ') || '-'}</td><td>{formatLocalizedNumber(Number(v.current_stock_quantity ?? 0), locale)}</td></tr>)}</tbody></table></div></section>;
}
...
function HierarchyPanel({ locations, canWrite }: { locations: Location[]; canWrite: boolean }) {
  ...
  return <section className="section" style={panelStyle}><div className="section__title">{ui("Location hierarchy")}</div>
    <div className="card"><p className="card__subtext capability-help">{ui("Organize existing storage locations into warehouse, zone, aisle, rack, shelf, bin, or storage levels. Changing this structure does not move stock; it changes only how locations are organized and whether they are pickable.")}</p><div style={formGridStyle}><label>{ui("Location")}<select value={locationId} onChange={(e) => setLocationId(e.target.value)}><option value="">{ui("Select…")}</option>{locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label>{ui("Parent")}<select value={parentId} onChange={(e) => setParentId(e.target.value)}><option value="">{ui("Top level")}</option>{locations.filter((l) => l.id !== locationId).map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label>{ui("Type")}<select value={type} onChange={(e) => setType(e.target.value)}><option value="warehouse">{ui("Warehouse")}</option><option value="zone">{ui("Zone")}</option><option value="aisle">{ui("Aisle")}</option><option value="rack">{ui("Rack")}</option><option value="shelf">{ui("Shelf")}</option><option value="bin">{ui("Bin")}</option><option value="storage">{ui("Storage")}</option></select></label><label>{ui("Code")}<input value={code} onChange={(e) => setCode(e.target.value)} placeholder="WH1-A03-R02-B04" /></label><label><input type="checkbox" checked={pickable} onChange={(e) => setPickable(e.target.checked)} /> {ui("Pickable location")}</label><button className="button" type="button" disabled={!canWrite || !locationId || save.isPending} onClick={() => save.mutate()}>{save.isPending ? ui("Saving…") : ui("Save hierarchy")}</button></div>{error ? <div className="form-error">{error}</div> : null}</div>
    <div className="card" style={tableWrapStyle}><table><thead><tr><th>{ui("Path")}</th><th>{ui("Type")}</th><th>{ui("Code")}</th><th>{ui("Stock")}</th><th>{ui("Pickable")}</th></tr></thead><tbody>{!(hierarchy.data || []).length ? <EmptyTableRow colSpan={5} message={ui("No storage locations are available to organize.")} /> : null}{(hierarchy.data || []).map((l) => <tr key={l.id}><td style={{ paddingLeft: 14 + Number(l.depth || 0) * 14 }}>{l.path || l.name}</td><td>{displayCanonicalLabel(l.location_type, ui)}</td><td>{l.location_code || '—'}</td><td>{formatLocalizedNumber(Number(l.stock_quantity || 0), locale)}</td><td>{l.is_pickable ? ui("Yes") : ui("No")}</td></tr>)}</tbody></table></div>
  </section>;
}
...
function BomPanel({ products, locations, canWrite, canExecute }: { products: Product[]; locations: Location[]; canWrite: boolean; canExecute: boolean }) {
  ...
  const create = useMutation({ mutationFn: () => apiRequest('/inventory-capabilities/boms', { method: 'POST', body: JSON.stringify({ product_id: outputProductId, name, output_quantity: Number(outputQty), components: selectedComponents.map((c) => ({ product_id: c.product_id, quantity: Number(c.quantity), waste_percent: Number(c.waste_percent || 0) })) }) }), onSuccess: () => { setMessage(ui("BOM created successfully.")); setError(null); void qc.invalidateQueries({ queryKey: ['inventory-boms'] }); }, onError: (e) => { setMessage(null); setError(messageFrom(e, ui("Unable to create BOM."))); } });
  const execute = useMutation({ mutationFn: () => apiRequest(`/inventory-capabilities/boms/${executeBomId}/execute`, { method: 'POST', body: JSON.stringify({ direction, storage_location_id: executeLocationId, output_quantity: Number(executeQty), reservation_shortfall_acknowledged: false }) }), onSuccess: () => { setMessage(`${direction === 'assemble' ? ui('Assembly') : ui('Disassembly')} ${ui('completed and stock movements recorded.')}`); setError(null); void qc.invalidateQueries({ queryKey: ['inventory-capabilities-products'] }); }, onError: (e) => { setMessage(null); setError(messageFrom(e, ui("Unable to execute BOM."))); } });
  ...
  return <section className="section" style={panelStyle}><div className="section__title">{ui("BOM, kits, assembly and disassembly")}</div>
    ...
    <div className="card"><h3>{ui("Assemble / disassemble")}</h3><p className="card__subtext capability-help">{ui("This is a stock-changing action. The app validates permissions, available stock, reservations, lot integrity, and serial-tracking rules before posting movements.")}</p><div style={formGridStyle}><label>{ui("BOM")}<select value={executeBomId} onChange={(e) => setExecuteBomId(e.target.value)}><option value="">{ui("Select…")}</option>{(boms.data || []).map((b) => <option key={b.id} value={b.id}>{b.product_sku} — {b.name}</option>)}</select></label><label>{ui("Location")}<select value={executeLocationId} onChange={(e) => setExecuteLocationId(e.target.value)}><option value="">{ui("Select…")}</option>{locations.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label>{ui("Action")}<select value={direction} onChange={(e) => setDirection(e.target.value)}><option value="assemble">{ui("Assemble")}</option><option value="disassemble">{ui("Disassemble")}</option></select></label><label>{ui("Finished quantity")}<input type="number" min="0.000001" step="any" value={executeQty} onChange={(e) => setExecuteQty(e.target.value)} /></label><button className="button" type="button" disabled={!canExecute || !executeBomId || !executeLocationId || asNumber(executeQty) <= 0 || create.isPending || execute.isPending} onClick={requestExecution}>{execute.isPending ? (direction === 'assemble' ? ui("Assembling…") : ui("Disassembling…")) : direction === 'assemble' ? ui("Assemble stock") : ui("Disassemble stock")}</button></div>{message ? <div className="form-success">{message}</div> : null}{error ? <div className="form-error">{error}</div> : null}</div>
    <div className="card" style={tableWrapStyle}><table><thead><tr><th>{ui("Output")}</th><th>{ui("BOM")}</th><th>{ui("Output qty")}</th><th>{ui("Components")}</th></tr></thead><tbody>{!(boms.data || []).length ? <EmptyTableRow colSpan={4} message={ui("No active BOMs have been created yet.")} /> : null}{(boms.data || []).map((b) => <tr key={b.id}><td>{b.product_sku} — {b.product_name}</td><td>{b.name}</td><td>{formatLocalizedNumber(Number(b.output_quantity), locale)}</td><td>{b.components.map((c) => `${c.component_sku || ''} ${c.component_name || ''} × ${formatLocalizedNumber(Number(c.quantity), locale)}`).join('; ')}</td></tr>)}</tbody></table></div>
  </section>;
}
