...
function CustomFieldsPanel({ products, canWrite }: { products: Product[]; canWrite: boolean }) {
  ...
  useEffect(() => { if (entityValues.data) { const next: Record<string, string> = {}; entityValues.data.forEach((row) => { next[row.field_key] = row.value === null || row.value === undefined ? '' : String(row.value); }); setValues(next); } }, [entityValues.data]);
