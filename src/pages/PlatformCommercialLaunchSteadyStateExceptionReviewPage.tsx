...
type ExceptionRow = {
  ...
  source_cadence_statuses: Array<{ code: string; status: string; cadence: string }>;
...
function evidenceLinksForRow(row: ExceptionRow): PageLink[] {
...
function badgeStyle(value: string): CSSProperties {
  ...
  if (value.includes('high') || value.includes('waiting') || value.includes('manual') || value.includes('unreviewed')) return { ...styles.badge, background: '#fef3c7', color: '#92400e' };
    ...
    <div style={styles.page}>
      <section style={styles.header}>
        <div>
          ...
          <h1 style={styles.title}>Commercial Launch Steady-State Exception Review</h1>
          <p style={styles.description}>
            Step 232 turns operations cadence obligations into a read-only exception-review queue for missed cadence,
            platform-health regression, customer adoption risk, support SLA risk, billing entitlement exceptions,
            backup/restore gaps, deployment smoke-test gaps, incident-prevention gaps, and growth-governance exceptions.
          </p>
        </div>
        <div style={styles.headerMeta}>
          <span style={badgeStyle(data?.posture || 'loading')}>{humanize(data?.posture || 'loading')}</span>
          <span style={styles.generated}>{data?.generated_at ? new Date(data.generated_at).toLocaleString() : 'Not generated yet'}</span>
          <button type="button" style={styles.button} onClick={() => exceptionReview.refetch()} disabled={exceptionReview.isFetching}>
            {exceptionReview.isFetching ? 'Refreshing...' : exceptionReview.error ? 'Retry' : 'Refresh'}
          </button>
        </div>
      </section>

      {exceptionReview.isLoading ? <div style={styles.card}>Loading commercial launch steady-state exception review queue...</div> : null}
      {exceptionReview.error ? <div style={styles.error}>Failed to load commercial launch steady-state exception review queue.</div> : null}
      ...
      {data ? (
        <>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Exception dependency chain</h2>
          ...
          <section style={styles.card}>
            <h2 style={styles.sectionTitle}>Exception review rows</h2>
                ...
                <tbody>
                  {data.exception_rows.map((row) => (
                    <tr key={row.code}>
                      ...
                      <td style={styles.td}><span style={badgeStyle(row.exception_review_status)}>{humanize(row.exception_review_status)}</span></td>
                      <td style={styles.td}>{row.required_review_evidence.join(', ')}</td>
                      <td style={styles.td}><ul style={styles.list}>{row.exception_controls.map((control) => <li key={control}>{control}</li>)}</ul></td>
          ...
          <section style={styles.twoColumn}>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Exception review rules</h2>
              <ul style={styles.list}>{data.exception_review_rules.map((rule) => <li key={rule}>{rule}</li>)}</ul>
            </article>
            <article style={styles.card}>
              <h2 style={styles.sectionTitle}>Limitations</h2>
              <ul style={styles.list}>{data.exception_review_limitations.map((item) => <li key={item}>{item}</li>)}</ul>
            </article>
          </section>
