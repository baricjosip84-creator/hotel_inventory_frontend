        ...
        <section style={styles.panel}>
          ...
          {visiblePurchases.length === 0 ? <p style={styles.empty}>No supplier-purchase recommendation matches the current filters.</p> : <div style={styles.tableWrap}><table style={styles.table}><thead><tr><th>Product / location</th><th>Usable position</th><th>Target</th><th>Transfer cover</th><th>Remaining gap</th><th>Recommended order</th><th>Supplier</th><th>Decision</th><th>Final quantity</th><th>Reason</th><th>Draft PO</th></tr></thead><tbody>{visiblePurchases.map((row) => {
            ...
            return <tr key={row.id}>
              ...
              <td><input style={styles.compactInput} value={draft.reason} maxLength={2000} placeholder={reasonRequired(draft.decision) ? 'Reason required' : 'Optional note'} disabled={disabled} onChange={(event) => setDrafts((current) => ({ ...current, [key]: { ...draft, reason: event.target.value } }))}/></td>
              <td>{row.linked_purchase_order_id ? <a href={`/purchase-orders?purchaseOrderId=${row.linked_purchase_order_id}`}>{row.linked_purchase_order_number || String(row.linked_purchase_order_id).slice(0, 8)} · {readable(row.linked_purchase_order_status)}</a> : 'Not created'}</td>
            </tr>;
          })}</tbody></table></div>}
...
const styles: Record<string, CSSProperties> = {
  ...
  eyebrow: { fontSize: 12, fontWeight: 800, letterSpacing: 1.1, color: '#2563eb', textTransform: 'uppercase' },
  title: { margin: '4px 0 6px', fontSize: 32, lineHeight: 1.15, color: '#0f172a' },
  subtitle: { margin: 0, maxWidth: 980, color: '#64748b', lineHeight: 1.55 },
  panel: { background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: 18, minWidth: 0, boxShadow: '0 2px 10px rgba(15, 23, 42, 0.04)' },
  toolbar: { display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'end' },
  field: { display: 'grid', gap: 6, fontWeight: 700, fontSize: 13 },
  input: { minWidth: 150, padding: '10px 12px', border: '1px solid #cbd5e1', borderRadius: 8, background: '#fff', color: '#111827' },
  compactInput: { width: '100%', minWidth: 145, padding: '9px 10px', border: '1px solid #cbd5e1', borderRadius: 8, background: '#fff', color: '#111827' },
  qtyInput: { width: 105, padding: '9px 10px', border: '1px solid #cbd5e1', borderRadius: 8, background: '#fff', color: '#111827' },
  primaryButton: { padding: '10px 14px', border: '1px solid #2563eb', borderRadius: 8, background: '#2563eb', color: '#fff', fontWeight: 700, cursor: 'pointer' },
  secondaryButton: { padding: '10px 14px', border: '1px solid #cbd5e1', borderRadius: 8, background: '#fff', color: '#0f172a', fontWeight: 700, cursor: 'pointer' },
  dangerButton: { padding: '10px 14px', border: '1px solid #ef4444', borderRadius: 8, background: '#ef4444', color: '#fff', fontWeight: 700, cursor: 'pointer' },
  ...
  stat: { padding: 14, border: '1px solid #e2e8f0', borderRadius: 10, background: '#fff', display: 'grid', gap: 5, minWidth: 0 },
  statLabel: { color: '#64748b', fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.5 },
