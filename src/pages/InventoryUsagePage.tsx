...
import { archiveInventoryUsageTemplate, closeInventoryUsagePeriod, createInventoryUsageAttachment, consumeInventoryUsageTemplate, createInventoryUsageTemplate, fetchInventoryUsageAnomalies, fetchInventoryUsageExceptions, fetchInventoryUsageImpact, fetchInventoryUsageLogs, fetchInventoryUsagePeriodClosures, fetchInventoryUsageSummary, fetchInventoryUsageStorageLocations, fetchInventoryUsageTemplates, fetchInventoryUsageScheduledTemplates, fetchInventoryUsageTemplateReadiness, previewInventoryUsageByBarcode, recordInventoryUsageByBarcode, recordInventoryUsageBulk, reviewInventoryUsageLog, runDueInventoryUsageTemplates, reverseInventoryUsageLog, scanInventoryUsageAlerts } from './inventoryUsage/inventoryUsageApi';
import { DEFAULT_USAGE_FILTERS } from './inventoryUsage/inventoryUsageConfig';
import { getRoleCapabilities } from '../lib/permissions';
import type { InventoryUsageBarcodeRequest, InventoryUsageBulkLine, InventoryUsageBulkRequest, InventoryUsagePeriodClosureDraft, InventoryUsageTemplate, InventoryUsageTemplateDraft, InventoryUsageTemplateReadiness } from './inventoryUsage/inventoryUsageTypes';
...
export default function InventoryUsagePage() {
  ...
  const templateReadinessQuery = useQuery({
    queryKey: ['inventory-usage-template-readiness-page', templates.map((template) => template.id).join(',')],
    queryFn: async () => {
      const readinessPairs = await Promise.all(
        templates.map(async (template) => [template.id, await fetchInventoryUsageTemplateReadiness(template.id)] as const)
      );

      return readinessPairs.reduce<Record<string, InventoryUsageTemplateReadiness>>((acc, [templateId, readiness]) => {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<InventoryUsageTemplate | null>(null);

  const reverseUsageMutation = useMutation({
    mutationFn: ({ usageLogId, reversalReason }: { usageLogId: string; reversalReason: string }) => {
      return reverseInventoryUsageLog(usageLogId, reversalReason);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-summary-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-logs-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-exceptions-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-anomalies-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-impact-page'] });
    }
  });


  const reviewUsageMutation = useMutation({
    mutationFn: ({ usageLogId, reviewStatus, reviewNotes }: { usageLogId: string; reviewStatus: 'reviewed' | 'follow_up_required'; reviewNotes?: string }) => {
  ...
  const barcodePreviewMutation = useMutation({
    mutationFn: (payload: InventoryUsageBarcodeRequest) => previewInventoryUsageByBarcode(payload)
  });
  ...
  const barcodeUsageMutation = useMutation({
    mutationFn: (payload: InventoryUsageBarcodeRequest) => recordInventoryUsageByBarcode({
    ...
    onSuccess: (data, variables) => {
      ...
      if (data.usage?.id && variables.evidence_original_filename?.trim() && variables.evidence_stored_filename?.trim()) {
  ...
  const bulkUsageMutation = useMutation({
    mutationFn: (payload: InventoryUsageBulkRequest) => recordInventoryUsageBulk(payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-summary-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-logs-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-exceptions-page'] });
  ...
  const createTemplateMutation = useMutation({
    mutationFn: (payload: InventoryUsageTemplateDraft) => createInventoryUsageTemplate(payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-templates-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-template-readiness-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-scheduled-templates-page'] });
    }
  });


  const archiveTemplateMutation = useMutation({
    mutationFn: ({ templateId, reason }: { templateId: string; reason?: string }) => archiveInventoryUsageTemplate(templateId, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-templates-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-template-readiness-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-scheduled-templates-page'] });
  ...
  const runDueScheduledTemplatesMutation = useMutation({
    mutationFn: () => runDueInventoryUsageTemplates({ limit: 25 }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-summary-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-logs-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-exceptions-page'] });
  ...
  const consumeTemplateMutation = useMutation({
    mutationFn: ({ templateId }: { templateId: string }) => consumeInventoryUsageTemplate(templateId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-summary-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-logs-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-exceptions-page'] });
  ...
  const closePeriodMutation = useMutation({
    mutationFn: (payload: InventoryUsagePeriodClosureDraft) => closeInventoryUsagePeriod(payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-period-closures-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-summary-page'] });
      queryClient.invalidateQueries({ queryKey: ['inventory-usage-exceptions-page'] });
    }
  });
  ...
  const handlePreviewBarcodeUsage = (payload: InventoryUsageBarcodeRequest) => {
    barcodePreviewMutation.mutate({
      ...
      quantity: payload.quantity !== undefined && payload.quantity !== '' ? Number(payload.quantity) : undefined,
      ...
      evidence_original_filename: payload.evidence_original_filename?.trim() || undefined,
      evidence_stored_filename: payload.evidence_stored_filename?.trim() || undefined,
  ...
  const handleRecordBarcodeUsage = (payload: InventoryUsageBarcodeRequest) => {
    barcodeUsageMutation.mutate({
      ...
      missing_evidence_acknowledged: payload.missing_evidence_acknowledged || undefined,
      evidence_original_filename: payload.evidence_original_filename?.trim() || undefined,
      evidence_stored_filename: payload.evidence_stored_filename?.trim() || undefined,
      evidence_mime_type: payload.evidence_mime_type?.trim() || undefined,
      evidence_file_size_bytes: payload.evidence_file_size_bytes || undefined,
  ...
  const handleArchiveTemplate = (template: InventoryUsageTemplate) => {
    const reason = window.prompt(`Why are you archiving the usage template "${template.name}"?`);

    if (reason === null) {
      return;
    }

    archiveTemplateMutation.mutate({ templateId: template.id, reason: reason.trim() || undefined });
  };



  const handleRunDueScheduledTemplates = () => {
    const dueCount = Number(scheduledTemplatesQuery.data?.summary?.due_count || 0);
    const confirmed = window.confirm(`Record ${dueCount} due scheduled usage template${dueCount === 1 ? '' : 's'} now? This will deduct stock for every ready due template.`);
  ...
  const handleRecordTemplate = (template: InventoryUsageTemplate) => {
    const confirmed = window.confirm(`Record usage now from template "${template.name}"? This will deduct stock for ${template.items?.length || 0} template lines.`);
  ...
  const handleReverseUsage = (usageLogId: string) => {
    const reversalReason = window.prompt('Why are you reversing this usage entry?');

    if (!reversalReason || !reversalReason.trim()) {
      return;
    }

    reverseUsageMutation.mutate({ usageLogId, reversalReason: reversalReason.trim() });
  };


  const handleReviewUsage = (usageLogId: string, reviewStatus: 'reviewed' | 'follow_up_required') => {
    const promptMessage = reviewStatus === 'follow_up_required'
  ...
  return (
    <InventoryUsageDashboard
      ...
      periodClosuresError={periodClosuresQuery.isError ? (periodClosuresQuery.error as Error) : null}
      periodClosing={closePeriodMutation.isPending}
      periodCloseError={closePeriodMutation.isError ? (closePeriodMutation.error as Error) : null}
      periodCloseResult={closePeriodMutation.data || null}
      onClosePeriod={handleClosePeriod}
      templates={templates}
      templatesLoading={templatesQuery.isLoading || templateReadinessQuery.isLoading}
      scheduledTemplates={scheduledTemplatesQuery.data}
      scheduledTemplatesLoading={scheduledTemplatesQuery.isLoading}
      scheduledTemplatesError={scheduledTemplatesQuery.isError ? (scheduledTemplatesQuery.error as Error) : null}
      scheduledTemplatesRunning={runDueScheduledTemplatesMutation.isPending}
      scheduledTemplatesRunError={runDueScheduledTemplatesMutation.isError ? (runDueScheduledTemplatesMutation.error as Error) : null}
      scheduledTemplatesRunResult={runDueScheduledTemplatesMutation.data || null}
      onRunDueScheduledTemplates={handleRunDueScheduledTemplates}
      templatesError={templatesQuery.isError ? (templatesQuery.error as Error) : templateReadinessQuery.isError ? (templateReadinessQuery.error as Error) : null}
      templateCreating={createTemplateMutation.isPending}
      templateCreateError={createTemplateMutation.isError ? (createTemplateMutation.error as Error) : null}
      templateArchivingId={archiveTemplateMutation.variables?.templateId || null}
      templateArchiveError={archiveTemplateMutation.isError ? (archiveTemplateMutation.error as Error) : null}
      templateRecordingId={consumeTemplateMutation.variables?.templateId || null}
      templateRecordError={consumeTemplateMutation.isError ? (consumeTemplateMutation.error as Error) : null}
      ...
      barcodePreviewError={barcodePreviewMutation.isError ? (barcodePreviewMutation.error as Error) : null}
      barcodePreviewResult={barcodePreviewMutation.data || null}
      barcodeRecording={barcodeUsageMutation.isPending}
      barcodeError={barcodeUsageMutation.isError ? (barcodeUsageMutation.error as Error) : null}
      barcodeResult={barcodeUsageMutation.data || null}
      barcodeEvidenceLinking={barcodeEvidenceAttachmentMutation.isPending}
      barcodeEvidenceError={barcodeEvidenceAttachmentMutation.isError ? (barcodeEvidenceAttachmentMutation.error as Error) : null}
      ...
      storageLocationsError={storageLocationsQuery.isError ? (storageLocationsQuery.error as Error) : null}
      ...
      bulkError={bulkUsageMutation.isError ? (bulkUsageMutation.error as Error) : null}
      ...
      reverseError={reverseUsageMutation.isError ? (reverseUsageMutation.error as Error) : null}
      onReverseUsage={handleReverseUsage}
      reviewingUsageId={reviewUsageMutation.variables?.usageLogId || null}
      reviewError={reviewUsageMutation.isError ? (reviewUsageMutation.error as Error) : null}
