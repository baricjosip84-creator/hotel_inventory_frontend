...
export default function PlatformTenantCommunicationsPage() {
  ...
  const save = useMutation({
    mutationFn: () => {
      const payload = { ...form, occurred_at: form.occurred_at || null, follow_up_due_at: form.follow_up_due_at || null };
  ...
  const resolveFollowUp = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['platform', 'tenant-communications'] })
  });
  ...
  return (
    <div style={styles.page}>
      ...
      {canWrite ? (
        <section style={styles.card}>
          ...
          <div style={styles.actions}>
            ...
            <button style={styles.primaryButton} disabled={!form.subject || !form.summary || (!editingId && !form.tenant_id) || save.isPending} onClick={() => save.mutate()}>{editingId ? 'Save changes' : 'Log communication'}</button>
      ...
      <section style={styles.card}>
        ...
        {!communications.isLoading && !(communications.data?.communications || []).length ? <p style={styles.muted}>No communications found.</p> : null}
            ...
            <article key={item.id} style={{ ...styles.item, opacity: item.archived_at ? 0.6 : 1 }}>
              <div style={styles.itemTop}>
                ...
                <div style={styles.actions}>
                  ...
                  {canWrite ? <button style={styles.secondaryButton} onClick={() => { setEditingId(item.id); setForm({ tenant_id: item.tenant_id, channel: item.channel, direction: item.direction, subject: item.subject, summary: item.summary, external_reference: item.external_reference || '', contact_name: item.contact_name || '', contact_email: item.contact_email || '', occurred_at: inputDateTime(item.occurred_at), follow_up_required: item.follow_up_required, follow_up_due_at: inputDateTime(item.follow_up_due_at) }); }}>Edit</button> : null}
                  {canWrite && item.follow_up_required && !item.resolved_at ? <button style={styles.secondaryButton} onClick={() => resolveFollowUp.mutate(item.id)}>Resolve follow-up</button> : null}
                  {canWrite && !item.archived_at ? <button style={styles.dangerButton} onClick={() => archive.mutate(item.id)}>Archive</button> : null}
                </div>
              </div>
...
const styles: Record<string, CSSProperties> = {
  ...
  muted: { color: '#64748b', fontSize: 13 }, list: { display: 'grid', gap: 12 }, item: { border: '1px solid #e2e8f0', borderRadius: 12, padding: 12, background: '#f8fafc' }, itemTop: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }, body: { whiteSpace: 'pre-wrap', margin: '12px 0', color: '#0f172a' }, badge: { borderRadius: 999, padding: '5px 8px', background: '#fff7ed', color: '#9a3412', border: '1px solid #fed7aa', fontSize: 12 }
};
