...
export default function PlatformApiKeysPage() {
  ...
  const createKey = useMutation({
    mutationFn: () => platformApiRequest<CreateResponse>('/platform/api-keys', {
      ...
      body: JSON.stringify({
        ...
        scopes: form.scopes,
        allowed_ips: form.allowed_ips.split(',').map((ip) => ip.trim()).filter(Boolean),
  ...
  const revokeKey = useMutation({
    mutationFn: (apiKeyId: string) => platformApiRequest(`/platform/api-keys/${apiKeyId}/revoke`, { method: 'POST', body: JSON.stringify({ reason: revokeReason }) }),
    onSuccess: () => {
      setRevokeReason('');
      queryClient.invalidateQueries({ queryKey: ['platform', 'api-keys'] });
    }
      ...
      <section style={styles.card}>
        ...
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead><tr><th style={styles.th}>Tenant</th><th style={styles.th}>Name</th><th style={styles.th}>Prefix</th><th style={styles.th}>Scopes</th><th style={styles.th}>Last used</th><th style={styles.th}>Status</th><th style={styles.th}>Actions</th></tr></thead>
            <tbody>
              {rows.map((key) => (
                <tr key={key.id}>
                  <td style={styles.td}>{key.tenant_name || key.tenant_id}</td>
                  <td style={styles.td}><strong>{key.name}</strong><br /><span style={styles.help}>{key.description || 'No description'}</span></td>
                  ...
                  <td style={styles.td}><span style={badgeStyle(key)}>{statusLabel(key)}</span></td>
                  <td style={styles.td}>{canWrite && !key.revoked_at ? <div style={styles.actions}><button type="button" style={styles.secondaryButton} onClick={() => rotateKey.mutate(key.id)}>Rotate</button><input style={styles.reasonInput} value={revokeReason} onChange={(event) => setRevokeReason(event.target.value)} placeholder="Revoke reason" /><button type="button" style={styles.dangerButton} onClick={() => revokeKey.mutate(key.id)}>Revoke</button></div> : '—'}</td>
                </tr>
              ))}
              {!rows.length ? <tr><td style={styles.td} colSpan={7}>No API keys found.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </section>
