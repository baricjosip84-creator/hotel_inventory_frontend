...
function isValidDateTimeLocal(value: string) { return !value || !Number.isNaN(new Date(value).getTime()); }
...
function payload(form: JobForm) {
  ...
  return {
    name: form.name.trim(),
    job_key: form.job_key.trim().toLowerCase(),
    category: form.category,
    status: form.status,
    schedule_label: clean(form.schedule_label),
    owner_platform_user_id: clean(form.owner_platform_user_id),
    next_run_at: form.next_run_at ? new Date(form.next_run_at).toISOString() : null,
    failure_threshold: Number(form.failure_threshold || 3),
    runbook_id: clean(form.runbook_id),
    notes: clean(form.notes),
    worker_handler: clean(form.worker_handler),
    worker_payload: parsedWorkerPayload,
    ...
    recurrence_anchor_at: form.recurrence_anchor_at ? new Date(form.recurrence_anchor_at).toISOString() : null
  };
}
...
export default function PlatformOperationalJobsPage() {
  ...
  const refreshOperationalJobData = async (jobId?: string | null) => {
    ...
    await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs-due'] });
    await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-worker-heartbeats'] });
  };
  ...
  const refreshPage = async () => {
    ...
    await Promise.all([
      jobs.refetch(),
      dueJobs.refetch(),
      workerHeartbeats.refetch(),
      executionMetrics.refetch(),
      workerHandlers.refetch(),
      defaultJobs.refetch(),
      canWrite && canReadUsers ? users.refetch() : Promise.resolve(),
      canWrite && canReadRunbooks ? runbooks.refetch() : Promise.resolve()
    ]);
  };

  const save = useMutation({ mutationFn: () => { const wasEditing = Boolean(editingId); const body = JSON.stringify(payload(form)); return editingId ? platformApiRequest(`/platform/operational-jobs/${editingId}`, { method: 'PATCH', body }).then((saved) => ({ saved, wasEditing })) : platformApiRequest('/platform/operational-jobs', { method: 'POST', body }).then((created) => ({ created, wasEditing })); }, onSuccess: async (result: unknown) => { const wasEditing = typeof result === 'object' && result !== null && 'wasEditing' in result ? Boolean((result as { wasEditing?: boolean }).wasEditing) : Boolean(editingId); setForm(emptyForm); setEditingId(null); setMessage(wasEditing ? 'Operational job changes saved.' : 'Operational job created.'); if (!wasEditing) setFilters((prev) => ({ ...prev, attention_only: false })); await refreshOperationalJobData(); } });
  const recordRun = useMutation({ mutationFn: () => platformApiRequest(`/platform/operational-jobs/${runJobId}/runs`, { method: 'POST', body: JSON.stringify({ last_status: runForm.last_status, last_duration_ms: runForm.last_duration_ms ? Number(runForm.last_duration_ms) : null, last_error: clean(runForm.last_error), next_run_at: runForm.next_run_at ? new Date(runForm.next_run_at).toISOString() : null }) }), onSuccess: async () => { const recordedJobId = runJobId; setRunJobId(null); setRunForm(runFormDefault); setMessage('Job run result recorded.'); await refreshOperationalJobData(recordedJobId); } });
  const archive = useMutation({ mutationFn: (id: string) => platformApiRequest(`/platform/operational-jobs/${id}/archive`, { method: 'POST', body: JSON.stringify({}) }), onSuccess: async (_data, jobId) => { setMessage('Operational job archived.'); await refreshOperationalJobData(jobId); } });
  const claimJob = useMutation({ mutationFn: (id: string) => platformApiRequest<{ job: Job; run: { id: string } }>(`/platform/operational-jobs/${id}/claim`, { method: 'POST', body: JSON.stringify({ lease_seconds: 900, worker_id: 'platform-ui-manual-claim', worker_metadata: { source: 'platform_operational_jobs_ui', action: 'manual_claim' } }) }), onSuccess: async (_data, jobId) => { setMessage('Operational job claimed.'); await refreshOperationalJobData(jobId); } });
  const completeClaim = useMutation({ mutationFn: ({ id, runId, status }: { id: string; runId: string; status: 'success' | 'failed' | 'skipped' }) => platformApiRequest<Job>(`/platform/operational-jobs/${id}/complete-claim`, { method: 'POST', body: JSON.stringify({ run_id: runId, status, worker_id: 'platform-ui-manual-claim', worker_metadata: { source: 'platform_operational_jobs_ui', action: `manual_complete_${status}` } }) }), onSuccess: async (_data, variables) => { setMessage(variables.status === 'success' ? 'Claim completed as success.' : 'Claim completed as failed.'); await refreshOperationalJobData(variables.id); } });
  const releaseClaim = useMutation({ mutationFn: (id: string) => platformApiRequest<Job>(`/platform/operational-jobs/${id}/release-claim`, { method: 'POST', body: JSON.stringify({}) }), onSuccess: async (_data, jobId) => { setMessage('Operational job claim released.'); await refreshOperationalJobData(jobId); } });
  const scheduleRetry = useMutation({ mutationFn: (id: string) => platformApiRequest(`/platform/operational-jobs/${id}/schedule-retry`, { method: 'POST', body: JSON.stringify({ retry_at: null }) }), onSuccess: async (_data, jobId) => { setMessage('Retry scheduled for this operational job.'); await refreshOperationalJobData(jobId); } });
  const releaseExpiredClaims = useMutation({ mutationFn: () => platformApiRequest('/platform/operational-jobs/release-expired-claims', { method: 'POST', body: JSON.stringify({}) }), onSuccess: async () => { setMessage('Expired claims released.'); await refreshOperationalJobData(); } });
  const runWorkerOnce = useMutation({ mutationFn: () => platformApiRequest('/platform/operational-jobs/worker/run-once', { method: 'POST', body: JSON.stringify({ limit: 10, lease_seconds: 900 }) }), onSuccess: async () => { setMessage('Worker run-once completed.'); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs-due'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-worker-heartbeats'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-execution-metrics'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-job-runs'] }); } });
  const runHeartbeatCheck = useMutation({ mutationFn: () => platformApiRequest<WorkerHeartbeatCheck>('/platform/operational-jobs/scheduler/heartbeat-check', { method: 'POST', body: JSON.stringify({ stale_after_seconds: 300 }) }), onSuccess: async (result) => { setHeartbeatCheckResult(result); setMessage('Heartbeat check completed.'); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-worker-heartbeats'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-execution-metrics'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); } });
  const recoverStaleClaims = useMutation({ mutationFn: () => platformApiRequest('/platform/operational-jobs/recover-stale-claims', { method: 'POST', body: JSON.stringify({ older_than_seconds: 0 }) }), onSuccess: async () => { setMessage('Stale claims recovered.'); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-execution-metrics'] }); } });
  ...
  const invalidRecurrenceInterval = form.recurrence_type !== 'manual' && (!Number.isInteger(Number(form.recurrence_interval)) || Number(form.recurrence_interval) < 1 || Number(form.recurrence_interval) > 100000);
  const invalidNextRun = !isValidDateTimeLocal(form.next_run_at);
  const invalidRecurrenceAnchor = !isValidDateTimeLocal(form.recurrence_anchor_at);
  const invalidRunDuration = Boolean(runForm.last_duration_ms) && Number(runForm.last_duration_ms) < 0;
  const invalidRunNextRun = !isValidDateTimeLocal(runForm.next_run_at);
  const saveDisabled = save.isPending || !form.name.trim() || !form.job_key.trim() || invalidJobKey || invalidWorkerPayload || invalidFailureThreshold || invalidRecurrenceInterval || invalidNextRun || invalidRecurrenceAnchor;
  const saveHelp = !form.name.trim() || !form.job_key.trim()
    ? 'Job name and job key are required before creating an operational job.'
    : invalidJobKey
      ? 'Job key must be 3-120 lowercase-safe characters using letters, numbers, dot, underscore, colon or dash.'
      : invalidWorkerPayload
        ? 'Worker payload must be a valid JSON object.'
        : invalidFailureThreshold
          ? 'Failure threshold must be a whole number from 1 to 100.'
          : invalidRecurrenceInterval
            ? 'Recurring jobs need a whole recurrence interval from 1 to 100000.'
            : invalidNextRun || invalidRecurrenceAnchor
              ? 'Next run and recurrence anchor must be valid date/time values.'
              : '';
  const hasLoadError = jobs.error || dueJobs.error || workerHeartbeats.error || executionMetrics.error || workerHandlers.error || defaultJobs.error;
  ...
  return (
    <div style={styles.page}>
      <header style={styles.header}>
        <div>
          <h1 style={styles.title}>Operational jobs</h1>
          <p style={styles.subtitle}>Track recurring platform work such as backups, retention scans, billing syncs, webhook delivery, health scans, and SLA checks.</p>
        </div>
        <button type="button" style={styles.secondaryButton} onClick={() => { void refreshPage(); }} disabled={isRefreshing}>{isRefreshing ? 'Refreshing…' : 'Refresh'}</button>
      </header>

      <section style={styles.metaCard}>
        <span>Source: GET /platform/operational-jobs</span>
        <span>Execution: /platform/operational-jobs/execution-metrics?days=14</span>
        <span>Worker state: heartbeat + due jobs</span>
        <span>Limit: 300</span>
        <span>Visible jobs: {rows.length}</span>
        <span>Filters: {queryString || 'limit=300'}</span>
      </section>

      <section style={styles.linkCard}>
        <strong>Supporting Platform pages:</strong>
        <Link to="/platform/runbooks">Runbooks</Link>
      ...
      {hasLoadError ? <section style={styles.errorCard}><strong>Operational jobs data could not be fully loaded.</strong><span>{((jobs.error || dueJobs.error || workerHeartbeats.error || executionMetrics.error || workerHandlers.error || defaultJobs.error) as Error).message}</span><button type="button" style={styles.secondaryButton} onClick={() => { void refreshPage(); }}>Retry</button></section> : null}
      ...
      {canWrite ? <section id="platform-operational-jobs-form" style={styles.panel}>
        ...
        {saveHelp ? <p style={styles.validationText}>{saveHelp}</p> : null}{save.error ? <p style={styles.errorText}>{(save.error as Error).message}</p> : null}
        ...
        <label style={styles.fieldLabel}>Worker payload<textarea value={form.worker_payload} onChange={(event) => setForm((prev) => ({ ...prev, worker_payload: event.target.value }))} placeholder="{}" style={styles.textarea} /></label>
      ...
      <section style={styles.panel}><h2 style={styles.sectionTitle}>Jobs</h2>{jobs.isLoading ? <p>Loading jobs…</p> : null}{rows.length === 0 && !jobs.isLoading ? <p>No jobs found.</p> : null}<div style={styles.cards}>{rows.map((job) => <article key={job.id} style={styles.card}><div style={styles.cardTop}><div><strong>{job.name}</strong><div style={styles.muted}>{job.job_key}</div></div><span style={statusStyle(job)}>{label(job.status)}</span></div><div style={styles.details}><span>Category: {label(job.category)}</span><span>Schedule: {job.schedule_label || '—'}</span><span>Owner: {job.owner_email || '—'}</span><span>Runbook: {job.runbook_title || '—'}</span><span>Last run: {dateTime(job.last_run_at)}</span><span>Last status: {label(job.last_status)}</span><span>Duration: {job.last_duration_ms ?? '—'} ms</span><span>Next run: {dateTime(job.next_run_at)}</span><span>Failures: {job.consecutive_failures}/{job.failure_threshold}</span><span>Claimed by: {job.claimed_by_email || '—'}</span><span>Claim expires: {dateTime(job.claim_expires_at)}</span><span>Next retry: {dateTime(job.next_retry_at)}</span><span>Dead-lettered: {dateTime(job.dead_lettered_at)}</span></div><div style={styles.actions}><button type="button" onClick={() => setHistoryJobId((current) => current === job.id ? null : job.id)} style={styles.secondaryButton}>Execution history</button>{job.runbook_id ? <Link to="/platform/runbooks" style={styles.inlineLink}>Runbook evidence</Link> : null}<Link to="/platform/audit" style={styles.inlineLink}>Audit evidence</Link>{job.worker_handler ? <Link to="/platform/operational-jobs" style={styles.inlineLink}>Worker handler evidence</Link> : null}</div>{job.last_error && job.last_status === 'failed' ? <p style={styles.errorText}>{job.last_error}</p> : null}{job.notes ? <p style={styles.note}><strong>Notes:</strong> {job.notes}</p> : null}{canWrite ? <div style={styles.actions}><button type="button" onClick={() => { setEditingId(job.id); setForm(toForm(job)); scrollToFormSection('platform-operational-jobs-form'); }} style={styles.secondaryButton}>Edit</button><button type="button" onClick={() => { setRunJobId(job.id); setRunForm(runFormDefault); }} style={styles.secondaryButton}>Record run</button>{!job.current_run_id && job.status !== 'archived' ? <button type="button" onClick={() => { if (window.confirm('Claim this operational job for manual work?')) claimJob.mutate(job.id); }} disabled={claimJob.isPending} style={styles.secondaryButton}>Claim</button> : null}{job.current_run_id ? <button type="button" onClick={() => { if (window.confirm('Complete this claimed job as success?')) completeClaim.mutate({ id: job.id, runId: job.current_run_id as string, status: 'success' }); }} disabled={completeClaim.isPending} style={styles.primaryButton}>Complete success</button> : null}{job.current_run_id ? <button type="button" onClick={() => { if (window.confirm('Complete this claimed job as failed?')) completeClaim.mutate({ id: job.id, runId: job.current_run_id as string, status: 'failed' }); }} disabled={completeClaim.isPending} style={styles.dangerButton}>Complete failed</button> : null}{job.current_run_id ? <button type="button" onClick={() => { if (window.confirm('Release this active claim?')) releaseClaim.mutate(job.id); }} disabled={releaseClaim.isPending} style={styles.secondaryButton}>Release claim</button> : null}{job.dead_lettered_at || job.last_status === 'failed' ? <button type="button" onClick={() => { if (window.confirm('Schedule retry for this operational job now?')) scheduleRetry.mutate(job.id); }} disabled={scheduleRetry.isPending} style={styles.secondaryButton}>Retry now</button> : null}{job.status !== 'archived' ? <button type="button" onClick={() => { if (window.confirm('Archive this operational job?')) archive.mutate(job.id); }} style={styles.dangerButton}>Archive</button> : null}</div> : null}</article>)}</div></section>
    </div>
  );
}
...
const styles: Record<string, CSSProperties> = {
  ...
  linkCard: { display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '10px 12px', color: '#334155', fontSize: 13 },
  success: { background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#166534', borderRadius: 12, padding: '10px 12px', fontWeight: 700 },
  errorCard: { background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: 14, color: '#991b1b', display: 'flex', flexDirection: 'column', gap: 8 },
  inlineLink: { color: 'var(--io-primary-dark)', fontSize: 13, fontWeight: 700, textDecoration: 'none', alignSelf: 'center' },
  smallTitle: { margin: '0 0 0.5rem', fontSize: '1rem', color: '#0f172a' },
  simpleList: { display: 'grid', gap: '0.5rem', padding: 0, margin: 0, listStyle: 'none' },
  listItem: { display: 'grid', gap: '0.15rem', padding: '0.6rem', border: '1px solid #e2e8f0', borderRadius: 10, background: '#f8fafc', color: '#334155' },
  ...
  panel: { background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: 18, display: 'flex', flexDirection: 'column', gap: 12, boxShadow: '0 1px 2px rgba(15,23,42,.03), 0 8px 24px rgba(15,23,42,.04)', minWidth: 0 },
  ...
  grid4: { display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 10, alignItems: 'center' },
  ...
  textarea: { padding: '10px 12px', border: '1px solid #cbd5e1', borderRadius: 10, minHeight: 86, background: '#fff', color: '#0f172a' },
  checkRow: { display: 'flex', alignItems: 'center', gap: 8, color: '#334155' },
  actions: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' },
  primaryButton: { background: 'var(--io-primary)', color: '#fff', border: '1px solid var(--io-primary)', borderRadius: 9, padding: '9px 13px', cursor: 'pointer', fontWeight: 700, boxShadow: '0 1px 2px rgba(15,23,42,.05)' },
  secondaryButton: { background: '#fff', color: '#0f172a', border: '1px solid #cbd5e1', borderRadius: 9, padding: '8px 10px', cursor: 'pointer', fontWeight: 700 },
  dangerButton: { background: '#dc2626', color: '#fff', border: '1px solid #dc2626', borderRadius: 9, padding: '8px 10px', cursor: 'pointer', fontWeight: 700 },
  disabledButton: { opacity: 0.55, cursor: 'not-allowed' },
  validationText: { margin: 0, color: '#92400e', background: '#fffbeb', border: '1px solid #fde68a', padding: 10, borderRadius: 10, fontWeight: 700 },
  cards: { display: 'grid', gap: 12 },
  card: { border: '1px solid #e2e8f0', borderRadius: 12, padding: 14, display: 'flex', flexDirection: 'column', gap: 10, background: '#fff' },
  cardTop: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' },
  details: { display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 8, color: '#475569', fontSize: 13 },
  badge: { borderRadius: 999, padding: '4px 9px', background: 'var(--io-primary-soft-strong)', color: 'var(--io-primary-dark)', alignSelf: 'flex-start', fontWeight: 700 },
  badgeGood: { borderRadius: 999, padding: '4px 9px', background: '#dcfce7', color: '#166534', alignSelf: 'flex-start', fontWeight: 700 },
  badgeWarn: { borderRadius: 999, padding: '4px 9px', background: '#fef3c7', color: '#92400e', alignSelf: 'flex-start', fontWeight: 700 },
  badgeDanger: { borderRadius: 999, padding: '4px 9px', background: '#fee2e2', color: '#991b1b', alignSelf: 'flex-start', fontWeight: 700 },
  badgeMuted: { borderRadius: 999, padding: '4px 9px', background: '#e2e8f0', color: '#475569', alignSelf: 'flex-start', fontWeight: 700 },
  muted: { color: '#64748b', fontSize: 13 },
  errorText: { margin: 0, color: '#991b1b', background: '#fef2f2', border: '1px solid #fecaca', padding: 10, borderRadius: 10 },
  note: { margin: 0, color: '#475569', background: '#f8fafc', border: '1px solid #e2e8f0', padding: 10, borderRadius: 10 }
};
