...
export default function PlatformOperationalJobsPage() {
  ...
  const claimJob = useMutation({ mutationFn: (id: string) => platformApiRequest<{ run: { id: string } }>(`/platform/operational-jobs/${id}/claim`, { method: 'POST', body: JSON.stringify({ lease_seconds: 900, worker_id: 'platform-ui-manual-claim', worker_metadata: { source: 'platform_operational_jobs_ui', action: 'manual_claim' } }) }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-job-runs'] }); } });
  const completeClaim = useMutation({ mutationFn: ({ id, runId, status }: { id: string; runId: string; status: 'success' | 'failed' | 'skipped' }) => platformApiRequest(`/platform/operational-jobs/${id}/complete-claim`, { method: 'POST', body: JSON.stringify({ run_id: runId, status, worker_id: 'platform-ui-manual-claim', worker_metadata: { source: 'platform_operational_jobs_ui', action: `manual_complete_${status}` } }) }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-job-runs'] }); } });
  const releaseClaim = useMutation({ mutationFn: (id: string) => platformApiRequest(`/platform/operational-jobs/${id}/release-claim`, { method: 'POST', body: JSON.stringify({}) }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-job-runs'] }); } });
  const scheduleRetry = useMutation({ mutationFn: (id: string) => platformApiRequest(`/platform/operational-jobs/${id}/schedule-retry`, { method: 'POST', body: JSON.stringify({ retry_at: null }) }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-job-runs'] }); } });
  const releaseExpiredClaims = useMutation({ mutationFn: () => platformApiRequest('/platform/operational-jobs/release-expired-claims', { method: 'POST', body: JSON.stringify({}) }), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-jobs'] }); await queryClient.invalidateQueries({ queryKey: ['platform', 'operational-job-runs'] }); } });
