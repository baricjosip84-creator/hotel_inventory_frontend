...
function evidenceLinksForRow(row: RetentionRenewalAcceptanceDocketRow): PageLink[] {
    ...
    <div style={styles.page}>
      <section style={styles.header}>
        ...
        <div style={styles.headerMeta}>
          ...
          <button type="button" style={styles.button} onClick={() => retentionRenewalAcceptanceDocket.refetch()} disabled={retentionRenewalAcceptanceDocket.isFetching}>
            {retentionRenewalAcceptanceDocket.isFetching ? 'Refreshing...' : retentionRenewalAcceptanceDocket.error ? 'Retry' : 'Refresh'}
          </button>
        </div>
      </section>
            ...
            <div style={styles.chainGrid}>
              {sourcePostureLinks.map((link) => {
                const postureMap: Record<string, string> = {
                  'Retention renewal review': data.retention_renewal_review_posture,
...
const styles: Record<string, CSSProperties> = {
  ...
  chainGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 10, color: '#475569' },
