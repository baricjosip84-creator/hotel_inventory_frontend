...
import './PlatformCommercialLaunchRetentionRenewalAcceptanceDocketPage.css';
...
type RetentionRenewalAcceptanceDocket = {
  ...
  retention_renewal_acceptance_persistence: PersistenceBoundary;
...
const summaryLabels: Record<string, string> = {
  retention_renewal_acceptance_rows_total: 'Acceptance docket rows',
  waiting_for_external_retention_renewal_review_confirmation: 'Blocked by external renewal-review confirmation',
  waiting_for_manual_retention_renewal_acceptance: 'Awaiting external manual acceptance',
  critical_acceptance_rows: 'Critical acceptance templates',
  high_acceptance_rows: 'High acceptance templates',
  medium_acceptance_rows: 'Medium acceptance templates',
  retention_renewal_acceptance_records_persisted_in_application: 'Acceptance records stored here'
};

const summaryHelpers: Record<string, string> = {
  retention_renewal_acceptance_rows_total: 'Read-only acceptance templates derived from Retention Renewal Review',
  waiting_for_external_retention_renewal_review_confirmation: 'Rows blocked until the external renewal-review record is independently confirmed',
  waiting_for_manual_retention_renewal_acceptance: 'Rows prepared only for an external manual acceptance record',
  critical_acceptance_rows: 'Template severity hints only; not observed external acceptance severity',
  high_acceptance_rows: 'Template severity hints only; not observed external acceptance severity',
  medium_acceptance_rows: 'Template severity hints only; not observed external acceptance severity',
  retention_renewal_acceptance_records_persisted_in_application: 'Expected to remain zero because this endpoint stores no external acceptance records or decisions'
};
...
export default function PlatformCommercialLaunchRetentionRenewalAcceptanceDocketPage() {
  const acceptanceDocket = useQuery({
  ...
  const data = acceptanceDocket.data;
  const summaryEntries = useMemo(() => Object.entries(data?.summary || {}), [data?.summary]);
  const initialLoadError = acceptanceDocket.isError && !data;
  const refreshError = acceptanceDocket.isError && Boolean(data);
  const requestError = readableError(acceptanceDocket.error);

  return (
    <div className="io-operational-page io-workspace-page platform-retention-renewal-acceptance">
      <OperationalWorkspaceHero
        iconPath="/platform/commercial-launch-retention-renewal-acceptance-docket"
        eyebrow="Platform Commercial Launch Readiness"
        title="Commercial Launch Retention Renewal Acceptance Docket"
        description="Read-only acceptance preparation after Retention Renewal Review. It organizes renewal-review packet locators, accountable-owner acceptance, leadership acknowledgement and next review dates without claiming that an external renewal review or acceptance decision has occurred."
        meta={<>
          <OperationalWorkspaceMetaPill>{data?.step || 'Step 241 — Commercial Launch Retention Renewal Acceptance Docket'}</OperationalWorkspaceMetaPill>
          <OperationalWorkspaceMetaPill>Retention renewal acceptance preparation only</OperationalWorkspaceMetaPill>
          <OperationalWorkspaceMetaPill>External renewal-review record required</OperationalWorkspaceMetaPill>
        </>}
        aside={
          <div className="platform-retention-renewal-acceptance__hero-aside">
            <OperationalWorkspaceStatus value={data ? data.summary.retention_renewal_acceptance_rows_total ?? data.retention_renewal_acceptance_docket_rows.length : '—'} label="acceptance rows" />
            {data ? <span className="platform-retention-renewal-acceptance__status-badge" data-tone={badgeTone(data.posture)}>{humanize(data.posture)}</span> : null}
            <div className="platform-retention-renewal-acceptance__refresh-block">
              <span>Last refreshed: {formatDateTime(data?.generated_at)}</span>
              <button type="button" className="app-button app-button--secondary" onClick={() => void acceptanceDocket.refetch()} disabled={acceptanceDocket.isFetching}>
                {acceptanceDocket.isFetching ? 'Refreshing…' : 'Refresh'}
              </button>
            </div>
          </div>
        }
      ...
      <section className="app-panel app-panel--padded platform-retention-renewal-acceptance__boundary-panel">
        <OperationalSectionHeader
          iconPath="/platform/commercial-launch-retention-renewal-acceptance-docket"
          title="External retention-renewal acceptance boundary"
          description="This docket prepares acceptance requirements; it cannot observe or persist the real external retention-renewal review outcome, renewal-review packet locator, accountable-owner acceptance, leadership acknowledgement, next review date, or acceptance decision."
        />
        <div className="platform-retention-renewal-acceptance__boundary-grid">
          <div className="platform-retention-renewal-acceptance__boundary-notice">
            <strong>Retention renewal acceptance preparation only.</strong>
            <span>Independently confirm the external Retention Renewal Review record before treating these acceptance rows as active. A zero stored-acceptance count does not prove that no external acceptance record exists.</span>
          </div>
          <div className="platform-retention-renewal-acceptance__supporting-pages">
            ...
            <div className="platform-retention-renewal-acceptance__link-row">
      ...
      {acceptanceDocket.isLoading ? <section className="app-panel app-panel--padded">Loading Commercial Launch Retention Renewal Acceptance Docket…</section> : null}

      {initialLoadError ? (
        <section className="app-error-state platform-retention-renewal-acceptance__feedback" role="alert">
          <strong>Unable to load Commercial Launch Retention Renewal Acceptance Docket.</strong>
          <span>{requestError}</span>
          <button type="button" className="app-button app-button--danger platform-retention-renewal-acceptance__retry" onClick={() => void acceptanceDocket.refetch()} disabled={acceptanceDocket.isFetching}>
            {acceptanceDocket.isFetching ? 'Retrying…' : 'Retry'}
          </button>
        </section>
      ) : null}

      {refreshError ? (
        <section className="app-panel app-panel--padded platform-retention-renewal-acceptance__feedback platform-retention-renewal-acceptance__feedback--warning" role="status">
          <strong>Refresh failed.</strong>
          <span>Showing the last successful Commercial Launch Retention Renewal Acceptance Docket snapshot. {requestError}</span>
          <button type="button" className="app-button app-button--secondary platform-retention-renewal-acceptance__retry" onClick={() => void acceptanceDocket.refetch()} disabled={acceptanceDocket.isFetching}>
            {acceptanceDocket.isFetching ? 'Retrying…' : 'Retry refresh'}
          </button>
        </section>
      ) : null}
      ...
      {data ? (
        <>
          <OperationalWorkspaceStats ariaLabel="Retention renewal acceptance docket summary">
            {summaryEntries.map(([key, value]) => (
              <OperationalWorkspaceStatCard key={key} iconPath="/platform/commercial-launch-retention-renewal-acceptance-docket" label={summaryLabels[key] || humanize(key)} value={value} helper={summaryHelpers[key] || 'Read-only derived snapshot metric'} />
            ))}
          </OperationalWorkspaceStats>

          <section className="app-panel app-panel--padded platform-retention-renewal-acceptance__context-panel">
            <OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-acceptance-docket" title="Current evidence chain" description="Current upstream posture and persistence context used to prepare the retention-renewal acceptance docket." />
            <div className="platform-retention-renewal-acceptance__source-grid">
            ...
            <div className="platform-retention-renewal-acceptance__persistence-grid">
              {[
          ...
          <section className="app-panel app-panel--padded platform-retention-renewal-acceptance__rows-section">
            <OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-acceptance-docket" title="Retention renewal acceptance rows" description="Prepared acceptance requirements. Status and severity are preparation metadata, not observed external renewal-review or acceptance outcomes." />
            {data.retention_renewal_acceptance_docket_rows.length === 0 ? (
              <div className="platform-retention-renewal-acceptance__empty-state">
                <strong>No retention-renewal acceptance rows were produced.</strong>
                <span>This is not evidence that retention renewal review or acceptance occurred externally. The derived docket remains blocked until source evidence can be established.</span>
              </div>
            ) : (
              <div className="platform-retention-renewal-acceptance__row-grid">
                {data.retention_renewal_acceptance_docket_rows.map((row) => (
                  <article key={row.code} className="app-panel platform-retention-renewal-acceptance__row-card">
                    <div className="platform-retention-renewal-acceptance__row-heading">
                      <div><h3>{humanize(row.code)}</h3><span>{humanize(row.domain)} · Owner: {humanize(row.owner)}</span></div>
                      <div className="platform-retention-renewal-acceptance__row-badges">
                        <span className="platform-retention-renewal-acceptance__status-badge" data-tone={badgeTone(row.severity_hint)}>{humanize(row.severity_hint)} severity hint</span>
                        <span className="platform-retention-renewal-acceptance__status-badge" data-tone={badgeTone(row.retention_renewal_acceptance_status)}>{humanize(row.retention_renewal_acceptance_status)}</span>
                      </div>
                    </div>
                    <div className="platform-retention-renewal-acceptance__template-note">Severity is a template hint; not an observed external acceptance severity. Acceptance status is preparation metadata only; not an observed external renewal-review or acceptance outcome.</div>
                    <div className="platform-retention-renewal-acceptance__source-summary">
                    ...
                    <div className="platform-retention-renewal-acceptance__source-rows"><strong>Source Operations Cadence statuses</strong><div className="platform-retention-renewal-acceptance__chips">{row.source_cadence_statuses.length ? row.source_cadence_statuses.map((source) => <span key={`${source.code}-${source.status}`}>{humanize(source.code)} · {humanize(source.status)} · {humanize(source.cadence)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-acceptance__source-rows"><strong>Source Steady-state Transition row references</strong><div className="platform-retention-renewal-acceptance__chips">{row.source_transition_rows.length ? row.source_transition_rows.map((value) => <span key={value}>{humanize(value)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-acceptance__source-rows"><strong>Source Additional Growth Observation row references</strong><div className="platform-retention-renewal-acceptance__chips">{row.source_observation_rows.length ? row.source_observation_rows.map((value) => <span key={value}>{humanize(value)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-acceptance__source-rows"><strong>Source Additional Growth Authorization row references</strong><div className="platform-retention-renewal-acceptance__chips">{row.source_authorization_rows.length ? row.source_authorization_rows.map((value) => <span key={value}>{humanize(value)}</span>) : <span>None reported</span>}</div></div>
                    <div className="platform-retention-renewal-acceptance__field-groups">
                      <div><strong>Required source recurrence-audit evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_recurrence_audit_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required source recurrence-resolution evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_recurrence_resolution_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required source resolution-verification evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_resolution_verification_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required source durable-closure certification evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_durable_closure_certification_evidence.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required external final-archive evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_final_evidence_archive.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required external retention-seal evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_evidence_retention_seal.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required external retention-renewal review evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_retention_renewal_review.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Required external retention-renewal acceptance evidence</strong><div className="platform-retention-renewal-acceptance__chips">{row.required_retention_renewal_acceptance.map((field) => <span key={field}>{humanize(field)}</span>)}</div></div>
                      <div><strong>Retention renewal acceptance controls</strong><ul>{row.retention_renewal_acceptance_controls.map((control) => <li key={control}>{control}</li>)}</ul></div>
                    </div>
                    <div className="platform-retention-renewal-acceptance__row-actions">
          ...
          <section className="platform-retention-renewal-acceptance__rules-grid">
            <div className="app-panel app-panel--padded"><OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-acceptance-docket" title="Retention renewal acceptance rules" description="Conditions that keep acceptance manual, externally evidenced and review-gated." /><ul>{data.retention_renewal_acceptance_rules.map((rule) => <li key={rule}>{rule}</li>)}</ul></div>
            <div className="app-panel app-panel--padded"><OperationalSectionHeader iconPath="/platform/commercial-launch-retention-renewal-acceptance-docket" title="Limitations" description="What this read-only acceptance docket does not observe, certify, persist or execute." /><ul>{data.retention_renewal_acceptance_limitations.map((item) => <li key={item}>{item}</li>)}</ul></div>
          </section>

          <section className="app-panel app-panel--padded platform-retention-renewal-acceptance__next-step"><strong>Next operator step</strong><span>{data.next_best_step}</span></section>
          <section className="app-panel app-panel--padded platform-retention-renewal-acceptance__snapshot-note"><strong>Snapshot interpretation</strong><span>{data.validation_note}</span><small>Generated {formatDateTime(data.generated_at)} · {data.phase}</small></section>
        </>
      ) : null}
    </div>
  );
